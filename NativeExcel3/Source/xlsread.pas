...
//////////////////////////////////////////////////////////////////////////////
//
//     Copyright (c) 2004-2012 NikaSoft. All rights reserved.
...
uses ole, xlsblob, xlshash, xlscalc, xlsvba{}, xlsescher{}, xlsstrm, xlschart,
     ...
     sysutils,  windows, wstream, xlsxread, xlscols, xlsrows, functransl,
     {$IFDEF D45}
         ComObj
     {$ELSE}
         Variants
...
type
  ...
  TXLSBlobArray =  xlsblob.TXLSBlobList;
   ...
   private
     ...
     FFunctionTranslator: TXLSFunctionTranslator;
     ...
     function ParseCountry(Data: TXLSBlob): integer;
...
begin
  ...
  try    //<<MD 23.07.12 fmShareDenyNone
...
begin
  ...
  if Assigned(Data[0]) then begin
    case RecID of
     ...
     {Country}     $008C: Result := ParseCountry(Data[0]);
...
begin
  Result := FBook._Extern.ParseSupBook(Data);
end;
...
begin
   offset := 6; 
   funcname := Data.GetBiffString(offset, true, true);
   if Assigned(FFunctionTranslator) then begin
      funcname := FFunctionTranslator.Translate(funcname);
   end;
   Result := FBook._Extern.SupBook.ParseExternalName(funcname);
end;
...
function TXLSReader.ParseCountry(Data: TXLSBlob): integer;
var code: word;
    tr: TXLSFunctionTranslator;
begin
  Result := 1;
  code := Data.GetWord(0);
  tr := GetFunctionTranslator(code);
  if Assigned(FFunctionTranslator) then FFunctionTranslator.Free;
  FFunctionTranslator := tr;
end;
...
begin
  ...
  if FFrozen then
     FBook.Sheets[FSheetCount].FreezePanes_(y, x, rowtop, colleft)
  else
