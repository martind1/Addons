...
uses
  SysUtils, Classes, xlshash, xlsBlob, xlsExtern, xlsstrb, xlsextbook;
       ...
       begin
         ...
         if Result = 1 then begin
            ...
            if Assigned(lFunc) then begin
                ...
                extrabookindex := FExtern.GetExtraBookIndex(lfuncname);
                if extrabookindex >= 0 then begin
                   lFunc := FExtern.GetExtraBookFunctionEntry(extrabookindex, lfuncname);
                end; 
                if Assigned(lFunc) then begin
                   Item.IntValue := 255;
                   lexternid := FExtern.GetExtraBookExternID(extrabookindex);
                   lnameid := FExtern.AddExtraFuncName(lexternid, lFunc.FName {function name});
                   if lnameid < 0 then begin
                      Result := -1;
                   end else begin
                      nm := TXLSSyntaxItem.Create();
                      nm.Value := lFunc.FName; {lfuncname}
                      nm.ItemType := SA_NAME;
                      nm.IntValue := lnameid;
...
begin
  ...
  if not(Assigned(lFuncInfo)) then begin
     if Item.IntValue = 255 then begin
        ...
        lFuncInfo := FExtern.GetExtraFuncEntry(lExternId, lNameId);
        if not(Assigned(lFuncInfo)) then begin
           Result := -1;
        end;
     end else begin
     ...
     $39, $59, $79 {tNameX}:
        begin
          ...
          if BiffVer < $0600 then begin
             ...
             //version >= biff8
             Item.FRangeValue.FExternID := wValue+1;
             //check FExternID
...
//function TXLSFormula.GetExtraFunctionPtg(Name: String): integer;
//begin
//   Result := FFuncs.GetExtraFunctionPtg(Name);
//end;
