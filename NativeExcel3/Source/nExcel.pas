...
uses Classes, Ole, SysUtils, xlsHash,
     {$IFDEF D45}
        ComObj,
     {$ELSE}
         Variants,
     ...
     xlsExtern, xlsextbook, xlshyper, xlsafilt, xlsdbcell, xlsstylecolor, xlscols,
     xlsrows, avltree;
...
{    procedure Group(Rows: boolean = true); 
  ...
  private
    ...
    procedure SetSplit(AValue: boolean);
    procedure SetSplitRowCol(ARow: double; ACol: double);
    procedure SetSplitColumn(ACol: double);
    procedure SetSplitRow(ARow: double);
    function GetActivePane: integer;
    procedure SetRowTop(AValue: integer);
    procedure SetColLeft(AValue: integer);
  public
    constructor Create(ASheet: IXLSWorksheet);
    procedure Clear;
    function StorePanes(DataList: TXLSBlobList; FileFormat: TXLSFileFormat): integer;
    property Split: boolean read FSplit write SetSplit;
    property FreezePanes: boolean read FFrozen write SetFrozen;
    property SplitColumn: double read FSplitColumn write SetSplitColumn;
    property SplitRow: double read FSplitRow write SetSplitRow;
  ...
  private
    ...
    function GetSplitRow: double;
    function GetSplitColumn: double;
    procedure SetSplitRow(ARow: double);
    procedure SetSplitColumn(ACol: double);
    function GetFreezePanes: boolean;
    function GetSplit: boolean;
    procedure SetFreezePanes(AValue: boolean);
    procedure SetSplit(AValue: boolean);
    function GetPaneScrollRow: integer;
    function GetPaneScrollColumn: integer;
    procedure SetPaneScrollRow(row: integer);
    procedure SetPaneScrollColumn(col: integer);
  public
    FSheetID: integer;
    property Cells: IXLSRange read GetCells;
    ...
    function FreezePanes_(row, col, RowTop, ColLeft: integer): integer;virtual;
    ...
    property  SplitRow: double read GetSplitRow write SetSplitRow;
    property  SplitColumn: double read GetSplitColumn write SetSplitColumn;
    property  FreezePanes: boolean read GetFreezePanes write SetFreezePanes;
    property  Split: boolean read GetSplit write SetSplit;
    property  PaneScrollRow: integer read GetPaneScrollRow write SetPaneScrollRow;
    property  PaneScrollColumn: integer read GetPaneScrollColumn write SetPaneScrollColumn;
...
uses xlsread, xlsstrm, 
    xlshtml, wstream, xlsrtf, xlsafit, xlscsv, xlsxwrite, math,
    {$IFDEF DXE2UP}Winapi.Windows{$ELSE}Windows{$ENDIF}
    ;
...
procedure TXLSWorkbook.RegisterExternalBook(ExtBook: TXLSExternalBook);
begin
  FExtern.RegisterExternalBook(ExtBook);
end;
...
begin
  ...
  Hr := FDocStorage.CreateStream( PWideChar(lName),
                             ...
                             Book);
  //Was is opened?
  if not(SUCCEEDED(Hr)) then Result := -1;
...
begin
  if (FFileName <> '') and (FFileFormat in [xlExcel5, xlExcel97, xlHTML, xlOpenXMLWorkbook] ) then begin
...
function IXLSWorksheet.FreezePanes_(row, col, RowTop,
  ColLeft: integer): integer;
begin
  Result := 1;
  if not(Assigned(FPanes)) then FPanes := TXLSPanes.Create(self);
...
var width, height: double;
    ...
    splitrow, splitcol: double;
    w, h: double;
begin

  if Not(Assigned(FPanes)) then begin
     FPanes := TXLSPanes.Create(self);
  end else begin
     FPanes.Clear;
  end;
  ...
  startcol := self.ScrollColumn;
  splitrow := 0;
  splitcol := 0;
  
  while (width > 0) do begin
     w := Cells[1, startcol].ColumnWidth;
     if (width > w) then begin 
        splitcol := splitcol + 1;
     end else begin
        splitcol := splitcol + width/w;
     end;
     width := width - w;                             	
     Inc(startcol);  
  end;   
  ...
  while (height > 0) do begin
     h := Cells[startrow, 1].RowHeight;
     if (height > h) then begin 
        splitrow := splitrow + 1;
     end else begin
        splitrow := splitrow + height/h;
     end;
     height := height - h;
     Inc(startrow);  
  end;   
  
  splitrow := Round(splitrow * 100) / 100;
  splitcol := Round(splitcol * 100) / 100;

  Self.SplitRow     := splitrow;
  Self.SplitColumn  := splitcol;
...
begin
   ...
   Data.AddWord(lSize - 4); //Number of bytes to follow
   frozen := false;
   if Assigned(FPanes) then frozen := FPanes.FreezePanes;
   grbit := 
                 $0 or                            //fDspFmla
                 ord(FScreenGridLines) shl 1  or  //fDspGrid
                 $1                    shl 2  or  //fDspRwCol
                 ord(frozen)          shl 3  or  //fFrozen
...
function IXLSWorksheet.StorePanes: integer;
begin
  if Assigned(FPanes) then begin
     Result := FPanes.StorePanes(FDataFooter, FWorkbook.FFileFormat);
  end else begin
     Result := 1;
  end;
end;
...
function IXLSWorksheet.GetSplitRow: double;
begin
  if Assigned(FPanes) then Result := FPanes.SplitRow else Result := 0;
end;

function IXLSWorksheet.GetSplitColumn: double;
begin
  if Assigned(FPanes) then Result := FPanes.SplitColumn else Result := 0;
end;

procedure IXLSWorksheet.SetSplitRow(ARow: double);
begin
  if not(Assigned(FPanes)) then FPanes := TXLSPanes.Create(self);
  FPanes.SplitRow := ARow; 
end;

procedure IXLSWorksheet.SetSplitColumn(ACol: double);
begin
  if not(Assigned(FPanes)) then FPanes := TXLSPanes.Create(self);
  FPanes.SplitColumn := ACol; 
end;

function IXLSWorksheet.GetFreezePanes: boolean;
begin
  if Assigned(FPanes) then Result := FPanes.FreezePanes else Result := false;
end;

function IXLSWorksheet.GetSplit: boolean;
begin
  if Assigned(FPanes) then Result := FPanes.Split else Result := false;
end;

procedure IXLSWorksheet.SetFreezePanes(AValue: boolean);
begin
  if not(Assigned(FPanes)) then FPanes := TXLSPanes.Create(self);
  FPanes.FreezePanes := AValue; 
end;

procedure IXLSWorksheet.SetSplit(AValue: boolean);
begin
  if not(Assigned(FPanes)) then FPanes := TXLSPanes.Create(self);
  FPanes.Split := AValue; 
end;             

function IXLSWorksheet.GetPaneScrollRow: integer;
begin
   //todo
   Result := 0;
end;

function IXLSWorksheet.GetPaneScrollColumn: integer;
begin
   //todo
  Result := 0;
end;

procedure IXLSWorksheet.SetPaneScrollRow(row: integer);
begin
   //todo
end;

procedure IXLSWorksheet.SetPaneScrollColumn(col: integer);
begin
   //todo
end;
...
constructor TXLSPanes.Create(ASheet: IXLSWorksheet);
begin
  inherited Create;
  FSheet := ASheet;
  FActivePane := 3;
...
function TXLSPanes.StorePanes(DataList: TXLSBlobList; FileFormat: TXLSFileFormat): integer;
Var Data: TXLSBlob;
    y, x, rwTop, colLeft, pnnAct: integer;
    rowcnt, colcnt: integer;
    rowsheight, colswidth: double;
    coef: double;
    i: integer;
begin
  Result := 1;
  if Split then begin
     if FreezePanes then begin
        //Frozen
        //Set default values for rwTop and colLeft
        y := Round(SplitRow);
        x := Round(SplitColumn);
        if FRowTop >= 0 then rwTop := FRowTop else rwTop := y;
        if FColLeft >= 0 then colLeft := FColLeft else colLeft := x;
     end else begin
        //Thawed 
        //Set default values for rwTop and colLeft

        rwTop := FSheet._RowInfo.GetFirstVisible();
        if rwTop < (FSheet.ScrollRow - 1) then begin
           rwTop := FSheet.ScrollRow - 1;
        end;

        colLeft := FSheet._ColumnInfo.GetFirstVisible();
        if colLeft < (FSheet.ScrollColumn - 1) then begin
           colLeft := FSheet.ScrollColumn - 1;
        end;

        rowcnt := Integer(Math.Floor(SplitRow));
        colcnt := Integer(Math.Floor(SplitColumn));

        rowsheight := 0; 
        if (rowcnt > 0) then begin
           rowsheight := FSheet.RCRange[rwTop + 1, 1, rwTop + rowcnt, 1].Height;
        end;

        coef := SplitRow - rowcnt;
        if (coef > 0) then begin
           rowsheight := rowsheight + coef * FSheet.Cells[rwTop + rowcnt + 1, 1].Height;
        end;   

        colswidth := 0; 
        if (colcnt > 0) then begin
            for i :=  colLeft + 1 to colLeft + colcnt do begin
                colswidth := colswidth + FSheet.FColumnInfo.Width[i - 1];
            end;
        end;
        coef := SplitColumn - colcnt;
        if (coef > 0) then begin
            colswidth := colswidth + coef * FSheet.FColumnInfo.Width[colLeft + colcnt];
        end;  
           
        y := 0;
        if rowsheight > 0 then begin
           y := Integer(Round(20 * rowsheight + 255));
        end;

        x := 0;
        if colswidth  > 0 then begin
           x := Integer(Round(113.879 * colswidth + 390));
        end;
        if FRowTop  >= 0 then rwTop   := FRowTop  else rwTop   := 0;
        if FColLeft >= 0 then colLeft := FColLeft else colLeft := 0;
     end;

     //Determine which pane should be active. 
     ...
     FActivePane := pnnAct; //Used in StoreSelection
     Data := TXLSBlob.Create(14);
     ...
     Result := FSheet.AppendData(Data, DataList);
  end;
end;



procedure TXLSPanes.SetFrozen(AValue: boolean);
begin
  if FFrozen and not(AValue) then begin
     FFrozen := false;
  end else if (not(FFrozen)) and AValue then begin
     if not(Split) then Split := true;
     if Split then begin
        FFrozen := true;
        FSplitRow := math.Ceil(FSplitRow);
        FSplitColumn := Math.Ceil(FSplitColumn);
        //FRowTop := Trunc(FSplitRow);
        //FColLeft := Trunc(FSplitColumn);
     end;
  end;
end;

procedure TXLSPanes.SetSplit(AValue: boolean);
var srow, scol: integer;
begin
   if FSplit and not(AValue) then begin
      FSplitRow := 0;
      FSplitColumn := 0;
      FSplit := false;
      FFrozen := false;   
   end else if (not(FSplit)) and AValue then begin
      srow := FSheet.Selection.Row - FSheet.ScrollRow;
      if (srow < 0) or (srow > 20) then srow := 0;
      scol := FSheet.Selection.Column - FSheet.ScrollColumn;
      if (scol < 0) or (scol > 10) then scol := 0;
      if (srow = 0) and (scol = 0) then begin
         srow := 1;
         scol := 1;
      end; 
...
procedure TXLSPanes.SetSplitRowCol(ARow: double; ACol: double);
begin
   if (Arow = 0) and (ACol = 0) then begin
      Split := false;
   end else if not(Split) then begin
      Split := true;
      FSplitRow := ARow;
      FSplitColumn := ACol;
      FRowTop  := Integer(Math.Floor(ARow)) + (FSheet.ScrollRow - 1);
      FColLeft := Integer(Math.Floor(ACol)) + (FSheet.ScrollColumn - 1);
   end else if FreezePanes then begin
      FSplitRow := Math.Ceil(ARow);
      FSplitColumn := Math.Ceil(ACol);
      FRowTop   := Trunc(FSplitRow) + (FSheet.ScrollRow - 1);
      FColLeft  := Trunc(FSplitColumn) + (FSheet.ScrollColumn - 1);
   end else begin
      FSplitRow := ARow;
      FSplitColumn := ACol;
      FRowTop  := Integer(Math.Floor(ARow)) + (FSheet.ScrollRow - 1);
...
procedure TXLSPanes.SetSplitColumn(ACol: double);
begin
  SetSplitRowCol(FSplitRow, ACol);
end;

procedure TXLSPanes.SetSplitRow(ARow: double);
begin
  SetSplitRowCol(ARow, FSplitColumn);
end;

function  TXLSPanes.GetActivePane: integer;
begin
  if Split then Result := FActivePane else Result := 3;
end;

procedure TXLSPanes.SetRowTop(AValue: integer);
begin
  if (AValue >= 0) and (AValue <= XLSMaxRow) then FRowTop := AValue;
end;


procedure TXLSPanes.SetColLeft(AValue: integer);
begin
  if (AValue >= 0) and (AValue <= XLSMaxCol) then FColLeft := AValue;
end;
...
function TXLSRange.GetHeight: double;
var r: integer;
begin
  Result := 0;
  for r := FFirstRow to FLastRow do begin
    Result := Result + FWorkSheet.FRowInfo.Height[r];  
  end;
end;
  ...
  case RangeType of
   ...
   2: begin
        ...
        if FWorkSheet.FColumnInfo.ColCount > 0 then begin
           if imin > FWorkSheet.FColumnInfo.MinCol then imin := FWorkSheet.FColumnInfo.MinCol;
           if imax < FWorkSheet.FColumnInfo.MaxCol then imax := FWorkSheet.FColumnInfo.MaxCol;
        end;
        ...
        for row := FFirstRow to FLastRow do begin
            ...
            for col := imin to imax do begin
               if Assigned(FWorkSheet.FCells.GetValue(row, col)) or 
                  FWorkSheet.FColumnInfo.IsColumnDefined(col) then 
               begin
                  CurXFIndex := GetXFIndex(row, col, ReplaceCurXFIndex);
                  if LastIndex <> CurXFIndex then begin
                     LastIndex := CurXFIndex;
            ...
            FWorkSheet.FRowInfo.XFIndex[row] := RowXFIndex;
        end;
      end;
    end;
end;
