...
type
  ...
  private
    ...
    //function  GetExtraFunctionId(Item: TXLSSyntaxItem): integer;
    function  CalcExtraFunction(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
    function  CalcExtraYield(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
    function  CalcExtraPrice(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
    function  CalcExtraCoupdays(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
    function  CalcExtraCoupdaybs(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
    function  CalcExtraCoupdaysnc(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
    function  CalcExtraCouppcd(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
    function  CalcExtraCoupncd(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
    function  CalcExtraCoupnum(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;

    function  Yield(Sattlement, Maturity: TDateTime; Rate, Pr, Redemption: Double; Frequency, Basis: integer): double; 
    function  Couppcd(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): TDateTime; 
    function  Coupncd(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): TDateTime; 
    function  IncMonth_(d: TDateTime; mc: integer; origIsLastDay: boolean; origDay: integer): TDateTime;
    function  Coupdays(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): double; 
    function  Coupdaysnc(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): double; 
    function  Coupdaybs(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): double; 
    function  MonthsBetween_(dt1, dt2: TDateTime): integer;
    function  DaysBetween_(dt1, dt2: TDateTime; Basis: integer): double; 
    function  Coupnum(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): double; 
    function  Price(Sattlement, Maturity: TDateTime; Rate, Yield, Redemption: double; Frequency, Basis: integer): double; 
...
implementation
uses Classes, sysutils, math, xlsmasks, xlsextbook, 
     {$IFDEF D45}
         ComObj
     {$ELSE}
         dateutils, Variants
...
function IncDay(value: TDateTime; days: integer): TDateTime;
begin
  Result := value + days;
end;


function DayOfTheMonth(dt: TDateTime): integer;
var y,m,d: word;
begin
  DecodeDate(dt, y, m, d);
  Result := d;
...
//>>md13.10.08
    ...
    255{extra functions}:
       Result := CalcExtraFunction(SheetIndex, Item, CurRow, CurCol, Value);
  end;
...
//function TXLSCalculator.GetExtraFunctionId(Item: TXLSSyntaxItem): integer;
//var funcname: widestring;
//begin
//  funcname := FExtern.GetExtraFuncName(Item.Child[1].Range.FExternID, Item.Child[1].IntValue);
//  if funcname <> '' then begin
//     Result := FFormula.GetExtraFunctionPtg(funcname);
//  end else begin
//     Result := -1;
//  end;
//  
...
function TXLSCalculator.CalcExtraFunction(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
var funcid: integer;
    funcentry: THashFuncEntry;
begin
  Value := Null;                      
  Result := xlsErrorNotSupported;
  funcid := -1;
  ...
  if Item.ChildCount < 1 then begin
     ...
     if FExtern.IsExternIDAddonFuncs(Item.Child[1].Range.FExternID) then begin
        funcentry := FExtern.GetExtraFuncEntry(Item.Child[1].Range.FExternID, Item.Child[1].IntValue);     
        if Assigned(funcentry) then funcid := funcentry.FPtg;
     end;     
  end;
  ...
  if funcid >= 0 then begin
     case funcid of 
       29: //EDATE
          Result := xlsErrorNotSupported;
       13: //COUPDAYBS
          Result := CalcExtraCoupdaybs(SheetIndex, Item, CurRow, CurCol, Value); 
       14: //COUPDAYS
          Result := CalcExtraCoupdays(SheetIndex, Item, CurRow, CurCol, Value); 
       15: //COUPDAYSNC
          Result := CalcExtraCoupdaysnc(SheetIndex, Item, CurRow, CurCol, Value); 
       16: //COUPNCD
          Result := CalcExtraCoupncd(SheetIndex, Item, CurRow, CurCol, Value); 
       17: //COUPNUM
          Result := CalcExtraCoupnum(SheetIndex, Item, CurRow, CurCol, Value); 
       18: //COUPPCD
          Result := CalcExtraCouppcd(SheetIndex, Item, CurRow, CurCol, Value); 
       74: //PRICE
          Result := CalcExtraPrice(SheetIndex, Item, CurRow, CurCol, Value); 
       90: //YIELD
          Result := CalcExtraYield(SheetIndex, Item, CurRow, CurCol, Value); 
       else
          Result := xlsErrorNotSupported;
     end;
  end;
...
function TXLSCalculator.CalcExtraYield(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
Var V: Variant;
    Sattlement, Maturity: TDateTime;
    Rate, Pr, Redemption: Double;
    Frequency, Basis: integer;
    lYield: double;
begin
  Result   := xlsOk;
  Value   := Null;
  ...
  //6-7 arguments + funcname
  if (Item.ChildCount < 7) or (Item.ChildCount > 8)  then begin
     Result := xlsErrorNA;
  end;
  ...
  if Result = xlsOk then begin
     ...
     Result := GetValueItem(SheetIndex, Item.Child[2], CurRow, CurCol, V, xlNumberValue);      
     if Result = xlsOk then begin
        Sattlement := V;
     end; 
  end;
  ...
  if Result = xlsOk then begin
     ...
     Result := GetValueItem(SheetIndex, Item.Child[4], CurRow, CurCol, V, xlNumberValue);      
     if Result = xlsOk then begin
        Rate := V;
     end; 
  end;
  ...
  if Result = xlsOk then begin
     ...
     Result := GetValueItem(SheetIndex, Item.Child[5], CurRow, CurCol, V, xlNumberValue);      
     if Result = xlsOk then begin
        Pr := V;
     end; 
  end;
  ...
  if Result = xlsOk then begin
     ...
     Result := GetValueItem(SheetIndex, Item.Child[6], CurRow, CurCol, V, xlNumberValue);      
     if Result = xlsOk then begin
        Redemption := V;
     end; 
  end;
  ...
  if Result = xlsOk then begin
     ...
     Result := GetValueItem(SheetIndex, Item.Child[7], CurRow, CurCol, V, xlNumberValue);      
     if Result = xlsOk then begin
        Frequency := Integer(V);
     end; 
  end;

  if (Result = xlsOk) and (Item.ChildCount = 8) then begin
     ...
     // 3 	Actual/365
     // 4 	European 30/360
     Result := GetValueItem(SheetIndex, Item.Child[8], CurRow, CurCol, V, xlNumberValue);      
     if Result = xlsOk then begin
        Basis := Integer(V);
     end; 
  end else begin
  ...
  if Result = xlsOK then begin
     Value := Null;
     Sattlement := Trunc(Sattlement);
     Maturity := Trunc(Maturity);
     if Rate < 0 then Result := xlsErrorNum;
     if Result = xlsOk then begin
        if Pr <= 0 then Result := xlsErrorNum;
        if Redemption <= 0 then Result := xlsErrorNum;
     end;
     if Result = xlsOk then begin
        if not((Frequency = 1) or (Frequency = 2) or (Frequency = 4)) 
        then Result := xlsErrorNum;
     end;
     if Result = xlsOk then begin
        if (Basis < 0) or (Basis > 4) then Result := xlsErrorNum;
     end;
     if Result = xlsOk then begin
        if Sattlement > Maturity then Result := xlsErrorNum;
     end;
     if Result = xlsOk then begin
        lYield := Yield(Sattlement, Maturity, Rate, Pr, Redemption, Frequency, Basis);
        if lYield < 0 then Result := xlsErrorNum;
        if Result = xlsOk then Value := lYield;
     end; 
  end;

end;
function TXLSCalculator.CalcExtraPrice(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
Var V: Variant;
    Sattlement, Maturity: TDateTime;
    Rate, Yld, Redemption: Double;
    Frequency, Basis: integer;
    lPrice: double;
begin
  Result   := xlsOk;
  ...
  if Result = xlsOK then begin
     ...
     if Rate < 0 then Result := xlsErrorNum;
     if Result = xlsOk then begin
        if Yld < 0 then Result := xlsErrorNum;
        if Redemption <= 0 then Result := xlsErrorNum;
     end;
     if Result = xlsOk then begin
        if not((Frequency = 1) or (Frequency = 2) or (Frequency = 4)) 
     ...
     if Result = xlsOk then begin
        lPrice := Price(Sattlement, Maturity, Rate, Yld, Redemption, Frequency, Basis);
        Value := lPrice;
     end; 
  end;
...
function TXLSCalculator.CalcExtraCoupdays(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
Var V: Variant;
    Sattlement, Maturity: TDateTime;
    Frequency, Basis: integer;
    lRet: double;
begin
  ...
  //3-4 arguments + funcname
  if (Item.ChildCount < 4) or (Item.ChildCount > 5)  then begin
     Result := xlsErrorNA;
  end;
  ...
  if (Result = xlsOk) and (Item.ChildCount = 5) then begin
     //Basis     is the type of day count basis to use.
     //Basis 	Day count basis
     // 0 or omitted 	US (NASD) 30/360
     // 1 	Actual/actual
  ...
  if Result = xlsOK then begin
     ...
     if Result = xlsOk then begin
        lRet := Coupdays(Sattlement, Maturity, Frequency, Basis);
        Value := lRet;
     end; 
  end;
...
function TXLSCalculator.CalcExtraCoupdaysnc(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
Var V: Variant;
    Sattlement, Maturity: TDateTime;
    Frequency, Basis: integer;
    lRet: double;
begin
  ...
  if Result = xlsOK then begin
     ...
     if Result = xlsOk then begin
        lRet := Coupdaysnc(Sattlement, Maturity, Frequency, Basis);
        Value := lRet;
     end; 
  end;
...
function TXLSCalculator.CalcExtraCoupdaybs(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
Var V: Variant;
    Sattlement, Maturity: TDateTime;
    Frequency, Basis: integer;
    lRet: double;
begin
  ...
  if Result = xlsOK then begin
     ...
     if Result = xlsOk then begin
        lRet := Coupdaybs(Sattlement, Maturity, Frequency, Basis);
        Value := lRet;
     end; 
  end;
...
function TXLSCalculator.CalcExtraCoupncd(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
Var V: Variant;
    Sattlement, Maturity: TDateTime;
    Frequency, Basis: integer;
    lRet: TDateTime;
begin
  ...
  if Result = xlsOK then begin
     ...
     if Result = xlsOk then begin
        lRet := Coupncd(Sattlement, Maturity, Frequency, Basis);
        Value := VarFromDateTime(lRet);
     end; 
  end;

end;

function TXLSCalculator.CalcExtraCouppcd(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
Var V: Variant;
    Sattlement, Maturity: TDateTime;
    Frequency, Basis: integer;
    lRet: TDateTime;
begin
  ...
  if Result = xlsOK then begin
     ...
     if Result = xlsOk then begin
        lRet := Couppcd(Sattlement, Maturity, Frequency, Basis);
        Value := VarFromDateTime(lRet);
     end; 
  end;
...
function TXLSCalculator.CalcExtraCoupnum(SheetIndex: integer; Item: TXLSSyntaxItem; CurRow, CurCol: integer; Var Value: Variant): integer;
Var V: Variant;
    Sattlement, Maturity: TDateTime;
    Frequency, Basis: integer;
    lRet: double;
begin
  ...
  if Result = xlsOK then begin
     ...
     if Result = xlsOk then begin
        lRet := Coupnum(Sattlement, Maturity, Frequency, Basis);
        Value := lRet;
     end; 
  end;
...
function TXLSCalculator.Yield(Sattlement, Maturity: TDateTime; Rate, Pr, Redemption: Double; Frequency, Basis: integer): double; 
var lYield1, lYield2, lYieldN: double;
    lPrice1, lPrice2, lPriceN: double;
    i: integer;
begin
  lYield1 := 0.0;
  lPrice1 := Price(Sattlement, Maturity, Rate, lYield1, Redemption, Frequency, Basis);

  lYield2 := 1.0;
  lPrice2 := Price(Sattlement, Maturity, Rate, lYield2, Redemption, Frequency, Basis);
                                     
  lYieldN := 0.5;
  Result := -1;

  for i := 0 to 99 do begin
     lPriceN := Price(Sattlement, Maturity, Rate, lYieldN, Redemption, Frequency, Basis);
     if (Pr = lPrice1) then begin
        Result := lYield1;
        break;
     end else if (Pr = lPrice2) then begin
        Result := lYield2;
        break;
     end else if (Pr = lPriceN) then begin
        Result := lYieldN;
        break;
     end else if (Pr < lPrice2) then begin
        lYield2 := lYield2 * 2;
        lPrice2 := Price(Sattlement, Maturity, Rate, lYield2, Redemption, Frequency, Basis);
        lYieldN := (lYield2 - lYield1)/2;
     end else begin
        if Pr < lPriceN then begin
           lYield1 := lYieldN;
           lPrice1 := lPriceN;
        end else begin
           lYield2 := lYieldN;
        ...
        lYieldN := lYield2 - (lYield2 - lYield1) * ((Pr - lPrice2) / (lPrice1 - lPrice2));
     end;
  end;

  if Result < 0 then begin
     if not(Abs(Pr - lPriceN) > (Pr/100)) then 
        Result := lYieldN;
  end;

end;

function TXLSCalculator.IncMonth_(d: TDateTime; mc: integer; origIsLastDay: boolean; origDay: integer): TDateTime;
var ldom, ldim: integer;
begin
  Result := IncMonth(d, mc);
  if OrigDay >= 28 then begin
     ldom := DayOfTheMonth(Result);
     ldim := DaysInMonth(Result);
     if origIsLastDay then begin
        if ldom < ldim then Result := IncDay(Result, ldim-ldom);
     end else begin
        if ldim > OrigDay then ldim := OrigDay; 
        if ldom <> ldim then Result := IncDay(Result, ldim-ldom);
     end; 
  end; 
end;

//COUPPCD Returns a number that represents the previous coupon date 
//before the settlement date.
function TXLSCalculator.Couppcd(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): TDateTime; 
var y1,m1,d1: word;
    y2,m2,d2: word;
    lOrigIsLastDay: boolean;
begin
  DecodeDate(Maturity, y2, m2, d2);
  DecodeDate(Sattlement, y1, m1, d1);
  lOrigIsLastDay := (d2 = DaysInMonth(Maturity)); 
  Result := IncMonth(Maturity, (y1 - y2) * 12);
  
  if Result < Sattlement then Result := IncMonth(Result, 12);
  while Result > Sattlement do Result := IncMonth_(Result, -12 div Frequency, lOrigIsLastDay, d2);
end;

//COUPNCD Returns a number that represents the next coupon date 
//after the settlement  date.
function TXLSCalculator.Coupncd(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): TDateTime; 
var y1,m1,d1: word;
    y2,m2,d2: word;
    lOrigIsLastDay: boolean;
begin
...
function TXLSCalculator.Coupdays(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): double; 
var db, da: TDateTime;
    y2, m2, d2: word;
begin
  case Basis of
        3 {fixed 365 days per year}: Result := 365 / Frequency;
        1 {actual days per year}: 
      begin
        db := Couppcd(Sattlement, Maturity, Frequency, Basis);
        DecodeDate(Maturity, y2, m2, d2);
        if d2 < 28 then begin
           da := IncMonth(db, 12 div Frequency);
        end else begin
           da := IncMonth_(db, 12 div Frequency, d2 = DaysInMonth(Maturity), d2);   
        end;
        Result := DaysBetween(db, da);
      end;
    else  {0,2,4 fixed 360 days per year} Result := 360 / Frequency; 
  end; 
end;


function TXLSCalculator.Coupdaysnc(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): double; 
var ncd: TDateTime;
begin
   case Basis of
     1,2,3: begin
               ncd := Coupncd(Sattlement, Maturity, Frequency, Basis);
               Result := DaysBetween(Sattlement, ncd);
            end;
     else Result := Coupdays(Sattlement, Maturity, Frequency, Basis) - 
                   Coupdaybs(Sattlement, Maturity, Frequency, Basis);
   end;
end;

function TXLSCalculator.Coupdaybs(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): double; 
var pcd: TDateTime;
begin
   pcd := Couppcd(Sattlement, Maturity, Frequency, Basis);
   Result := DaysBetween_(pcd, Sattlement, Basis);
end;

function TXLSCalculator.MonthsBetween_(dt1, dt2: TDateTime): integer;
var d1, m1, y1, d2, m2, y2: word;
begin
  DecodeDate(dt1, y1, m1, d1);
  DecodeDate(dt2, y2, m2, d2);
  Result := (12 - m1) + m2 + (y2 - y1 - 1) * 12;
end; 

function TXLSCalculator.DaysBetween_(dt1, dt2: TDateTime; Basis: integer): double; 
var tmp: TDateTime;
    dtm1, dtm2: TDateTime;
begin
  ...
  case Basis of 
    1,2,3: Result := DaysBetween(dt1, dt2);
    else begin
           dtm1 := IncDay(dt1, DaysInMonth(dt1) - DayOfTheMonth(dt1) + 1);
           if dtm1 < dt2 then begin
              Result := DaysBetween(dt1, dtm1);
              dtm2 := IncDay(dt2, -(DayOfTheMonth(dt2) - 1));
              if (dtm1 < dtm2) then begin
                 Result := Result + (MonthsBetween_(dtm1, dtm2) * 30);
              end;
              Result := Result + DayOfTheMonth(dt2) - 1;
           end else begin
...
function TXLSCalculator.Coupnum(Sattlement, Maturity: TDateTime; Frequency, Basis: integer): double; 
var pcd: TDateTime;
begin
   pcd := Couppcd(Sattlement, Maturity, Frequency, Basis);
   Result := (MonthsBetween_(pcd, Maturity) * Frequency) div 12;
end;


function  TXLSCalculator.Price(Sattlement, Maturity: TDateTime; Rate, Yield, Redemption: double; Frequency, Basis: integer): double; 
var lE, lDSC, lN, lA, lP1, lP2, lK: double;
begin
   lE := Coupdays(Sattlement, Maturity, Frequency, Basis);
   lDSC := Coupdaysnc(Sattlement, Maturity, Frequency, Basis);
   lN := Coupnum(Sattlement, Maturity, Frequency, Basis);
   lA := Coupdaybs(Sattlement, Maturity, Frequency, Basis);
   Result := Redemption / Power(1.0 + Yield/Frequency, lN - 1.0 + lDSC/lE);
   Result := Result - 100.0 * (Rate/Frequency) * (lA / lE);
   lK := 0;
   lP1 := 100 * Rate / Frequency;
   lP2 := 1 + Yield / Frequency;
   while lK < lN do begin
     Result := Result + lP1 / Power(lP2, lK + lDSC / lE);
     lK := lK + 1;
   end;
end;
