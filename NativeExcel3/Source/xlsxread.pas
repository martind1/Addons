...
//     function AttrValueAsBoolean(attrname: string): boolean; overload;
     ...
     function Parse_WS_f(r, c: integer): TXLSCompiledFormula;
...
//             Result := Parse_WS_v_e(e_value);
            ...
            is_formula := true;
            cf := Parse_WS_f(row, col);
            if not(Assigned(cf)) then begin
               is_formula := false;
            end;
        end else MoveToElementEnd;
     end;
  ...
  if Result = 1 then begin
     ...
     if is_formula then begin
        FSheet._SetCompiledFormula(row, col, cf);
     end;
  end;
...
function TXLSXReader.Parse_WS_f(r, c: integer): TXLSCompiledFormula;
var t: widestring;
    tid: integer;
    si: integer;
      ...
      if si >= 0 then begin
         if FSharedFormulaCells.KeyExists(si) then begin
             ...
             if Assigned(cf) then begin
                 //shared formula
                 Result := cf.GetCopy(r - msc.Row, c - msc.Col);
             end;
         end else begin
             msc := TXLSMasterSharedCell.Create(r, c);
             FSharedFormulaCells[si] := msc;
             sformula := ReadContent();
             Result := FBook._Formula.GetCompiledFormula(sFormula, FSheet.FSheetID); 
         end;
      end;
