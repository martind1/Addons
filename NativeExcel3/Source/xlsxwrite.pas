...
//////////////////////////////////////////////////////////////////////////////
//
//     Copyright (c) 2008-2013 NikaSoft. All rights reserved.
...
begin
  ...
  if isPaneExist then begin
     if FSheet.FreezePanes then begin
        ySplit := Round(FSheet._Panes.SplitRow);
        xSplit := Round(FSheet._Panes.SplitColumn);
        rwTop := ySplit;
        colLeft := xSplit;
        ...
        rowcnt := Trunc(FSheet._Panes.SplitRow);
        colcnt := Trunc(FSheet._Panes.SplitColumn);
        rowsheight := 0; 
        if (rowcnt > 0) then begin
           rowsheight := FSheet.RCRange[rwTop + 1, 1, rwTop + rowcnt, 1].Height;
        end; 
        coef := FSheet._Panes.SplitRow - rowcnt;
        ...
        if (colcnt > 0) then begin
            for i :=  colLeft + 1 to colLeft + colcnt do begin
                colswidth := colswidth + FSheet._ColumnInfo.Width[i - 1];
            end;
        end;

        coef := FSheet._Panes.SplitColumn - colcnt;
        if (coef > 0) then begin
            colswidth := colswidth + coef * FSheet._ColumnInfo.Width[colLeft + colcnt];
        end;  
        ...
        ySplit := 0;
        if rowsheight > 0 then ySplit := Round(20 * rowsheight + 255);
        xSplit := 0;
        if colswidth > 0 then xSplit := Round(113.879 * colswidth + 390);

        state := 'split';

        if FSheet._Panes.RowTop  >= 0 then rwTop   := FSheet._Panes.RowTop  else rwTop   := 0;
        if FSheet._Panes.ColLeft >= 0 then colLeft := FSheet._Panes.ColLeft else colLeft := 0;
...
begin
  ...
  FXMLWriter.WriteStartElement('col');
   ...
   if dval > 0 then begin
      SetAttrBoolean('customWidth', true);
