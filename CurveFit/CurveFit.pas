...
function PolyFit (const x, y: array of Double;
                   var coefs: array of Double;
                   var correl_coef: Double;
                   const npoints, nterms: Integer) : boolean;

{
  x,y                   : array of Double;  //Array der Messwerte
  var coefs             : array of Double;  //berechnete Koefffizienten, Reihenfolge x^0, x^1, x^2, ...
  var correl_coef       : Double;           //der Fehler
  const npoints, nterms : Integer         //Anzahl der Punkte, Kurve nterms-ter Ordnung anfitten
        ...
        if index [j, 2] <> 0 then
          ...
          for k := 0 to ncol - 1 do
           ...
           if index [k, 2] > 0 then
              begin
              error := true;
//              Raise EMathError.Create ('Error in GaussJordan2: matrix is singular');
              end;
            if index [k, 2] < 0 then
...
try  //ungültige Gleitkommaoperation (EInvalidOp)
  ...
  correl_coef := DivDef(srs, (sum_y2 - sqr (sum_y)/npoints), 1);
  if correl_coef >= 1
