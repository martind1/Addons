...
implementation
  ...
  function display_size(handle:Pointer;device:Pointer;width:integer;height:integer;raster:integer;format:UINT;pimage:PChar):integer;cdecl;
  begin
    ...
    Try
      ...
      Try
        ...
        Try
          ...
          SetLength(argv,8);
          argv[0]:=PChar(ParamStr(0));
          argv[1]:=PChar('-dNOPAUSE');
          argv[2]:=PChar('-dBATCH');
          argv[3]:=PChar('-sDEVICE='+'display');
          argv[4]:=PChar('-dDisplayFormat='+IntToStr(lFormat));
          argv[5]:=PChar('-r'+IntToStr(2));        //minimum resolution (for speed reasons)
          argv[6]:=PChar('-I' + PathToGSLib + ';' + PathToGSFonts + ';' + GetModulePath);
          argv[7]:=PChar(AFileName);
    ...
    Try
      ...
      Try
        ...
        Try
          ...
          argv[4]:=PChar('-dDisplayFormat='+IntToStr(lFormat));
          argv[5]:=PChar('-r'+IntToStr(2));        //minimum resolution (for speed reasons)
          argv[6]:=PChar('-dFirstPage=' + IntToStr(APage));
          argv[7]:=PChar('-dLastPage=' + IntToStr(APage));
          argv[8]:=PChar('-I' + PathToGSLib + ';' + PathToGSFonts + ';' + GetModulePath);
          argv[9]:=PChar(AFileName);
  ...
  begin
    ...
    Try
      ...
      Try
        ...
        argv[2]:=PChar('-dBATCH');
        argv[3]:=PChar('-sDEVICE='+'display');
        argv[4]:=PChar('-dDisplayFormat='+IntToStr(Fformat));
        argv[5]:=PChar('-r'+IntToStr(Round(FResolution*FZoom/100)));
        argv[6]:=PChar('-dFirstPage=' + IntToStr(APage));
        argv[7]:=PChar('-dLastPage=' + IntToStr(APage));
        argv[8]:=PChar('-dTextAlphaBits=4');
        argv[9]:=PChar('-sFONTPATH=' + GetFontPath);
        argv[10]:=PChar('-I' + PathToGSLib + ';' + PathToGSFonts + ';' + GetModulePath);
        argv[11]:=PChar(AFileName);
