...
(* //MD<<
06.03.14 md  Unicode fähig: Ansi Datentypen
*)
...
type
  ...
  TGSNewInstance         = function(pinstance:Pgs_main_instance;Ccaller_handle:Pointer):Integer; stdcall;
  TGSDeleteInstance      = procedure(pinstance:Pgs_main_instance); stdcall;
  TGSSetStdio            = function(pinstance:Pgs_main_instance; stdin_fn:TStdioFunction; stdout_fn:TStdioFunction;stderr_fn:TStdioFunction):Integer; stdcall;
  TGSSetPoll             = function(pinstance:Pgs_main_instance;poll_fn:TPollFunction):Integer; stdcall;
  TGSSetDisplayCallback  = function(pinstance:Pgs_main_instance;callback:PDisplayCallback):Integer; stdcall;
  TGSInitWithArgs        = function(pinstance:Pgs_main_instance;argc:integer;argv:PPAnsiChar):integer; stdcall;
  TGSRunStringBegin      = function(pinstance:Pgs_main_instance;user_errors:integer;pexit_code:Pinteger):integer; stdcall;
  TGSRunStringContinue   = function(pinstance:Pgs_main_instance;str:PAnsiChar;len:integer;user_errors:integer;pexit_code:pinteger):integer; stdcall;
  TGSRunStringEnd        = function(pinstance:Pgs_main_instance;user_errors:integer;pexit_code:pinteger):integer; stdcall;
  TGSRunStringWithLength = function(pinstance:Pgs_main_instance;str:PAnsiChar;len:integer;user_errors:integer;pexit_code:pinteger):integer; stdcall;
  TGSRunString           = function(pinstance:Pgs_main_instance;str:PAnsiChar;user_errors:integer;pexit_code:pinteger):integer; stdcall;
  TGSRunFile             = function(pinstance:Pgs_main_instance;file_name:PAnsiChar;user_errors:integer;pexit_code:pinteger):integer; stdcall;
  ...
  function  GSAPINewInstance(GSLib:HWnd;pinstance:Pgs_main_instance;caller_handle:Pointer):Integer;
  procedure GSAPIDeleteInstance(GSLib:HWnd;pinstance:Pgs_main_instance);
  function  GSAPISetStdio(GSLib:HWnd;pinstance:Pgs_main_instance; stdin_fn:TStdioFunction; stdout_fn:TStdioFunction;stderr_fn:TStdioFunction):Integer;
  function  GSAPISetPoll(GSLib:HWnd;pinstance:Pgs_main_instance; poll_fn:TPollFunction):Integer;
  function  GSAPISetDisplayCallback(GSLib:HWnd;pinstance:Pgs_main_instance;callback:PDisplayCallback):Integer;
  function  GSAPIInitWithArgs(GSLib:HWnd;pinstance:Pgs_main_instance;argc:integer;argv:PPAnsiChar):Integer;
  function  GSAPIRunStringBegin(GSLib:HWnd;pinstance:Pgs_main_instance;user_errors:integer;pexit_code:Pinteger):Integer;
  function  GSAPIRunStringContinue(GSLib:HWnd;pinstance:Pgs_main_instance;str:PAnsiChar;len:integer;user_errors:integer;pexit_code:pinteger):Integer;
  function  GSAPIRunStringEnd(GSLib:HWnd;pinstance:Pgs_main_instance;user_errors:integer;pexit_code:Pinteger):Integer;
  function  GSAPIRunStringWithLength(GSLib:HWnd;pinstance:Pgs_main_instance;str:PAnsiChar;len:integer;user_errors:integer;pexit_code:pinteger):Integer;
  function  GSAPIRunString(GSLib:HWnd;pinstance:Pgs_main_instance;str:PAnsiChar;user_errors:integer;pexit_code:pinteger):Integer;
  function  GSAPIRunFile(GSLib:HWnd;pinstance:Pgs_main_instance;file_name:PAnsiChar;user_errors:integer;pexit_code:pinteger):Integer;
  ...
  function image_size(width:integer;height:integer;raster:integer;format:UINT;pimage:PAnsiChar):boolean;
...
//------------------------------------------------------------------------------
procedure GSAPIError (const ltMessage:AnsiString);
var ltError:AnsiString;
begin
  if Length(ltMessage) > 1 then
  begin
    ltError := Copy(ltMessage, 1, 2);
    if UpperCase(ltError) = 'GS' then ltError := GS_ERROR_LOADING_FUNCTION + ltMessage
    else ltError:=ltMessage;

    Raise Exception.Create(PAnsiChar(ltMessage));
  end;
end;
...
//------------------------------------------------------------------------------
function GSAPILoadDll(const DllPath: AnsiString):HWnd;
var ltError,ltPath:String;
begin
  ltPath:= DllPath + gsdll32;
  Result := LoadLibrary(PChar(ltPath));
  {If an error occured, raise exception}
  if (Result < HINSTANCE_ERROR) then
  begin
    Result := 0;
    ltError := SysErrorMessage(GetLastError);
    if Trim(ltError) = '' then ltError:= GS_ERROR_LOADING + Chr(10) + 'Windows Error Number: ' + IntToStr(GetLastError)
    else ltError := GS_ERROR_LOADING + Chr(10) + 'Windows Error Number: ' + IntToStr(GetLastError) + ' - ' + Trim(ltError);
    GSAPIError(ltError);
  end;
end;
...
begin
  ...
  if GSLib = 0 then Exit;

  Result := FreeLibrary(GSLib);
  if Result = False then
  begin
    ltError:=SysErrorMessage(GetLastError);
    if Trim(ltError) = '' then GSAPIError(GS_ERROR_FREEING + Chr(10) + 'Windows Error Number: ' + IntToStr(GetLastError))
    else GSAPIError(GS_ERROR_FREEING + Chr(10) + 'Windows Error Number: ' + IntToStr(GetLastError) + ' - ' + Trim(ltError));
  end;
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_new_instance')
  else Result:=TGSRevision(pProc)(pr,len);
end;
...
begin
  ...
  pProc:=GetProcAddress(GSLib,'gsapi_new_instance');
  if (pProc = nil) then GSAPIError('gsapi_new_instance')
  else Result:=TGSNewInstance(pProc)(pinstance,caller_handle);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_delete_instance')
  else TGSDeleteInstance(pProc)(pinstance);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_set_stdio')
  else Result:=TGSSetStdio(pProc)(pinstance,stdin_fn,stdout_fn,stderr_fn);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_set_poll')
  else Result:=TGSSetPoll(pProc)(pinstance,poll_fn);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_set_display_callback')
  else Result:=TGSSetDisplayCallback(pProc)(pinstance,callback);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_init_with_args')
  else Result:=TGSInitWithArgs(pProc)(pinstance,argc,argv);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_run_string_begin')
  else Result:=TGSRunStringBegin(pProc)(pinstance,user_errors,pexit_code);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_run_string_continue')
  else Result:=TGSRunStringContinue(pProc)(pinstance,str,len,user_errors,pexit_code);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_run_string_end')
  else Result:=TGSRunStringEnd(pProc)(pinstance,user_errors,pexit_code);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_run_string_with_length')
  else Result:=TGSRunStringWithLength(pProc)(pinstance,str,len,user_errors,pexit_code);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_run_string')
  else Result:=TGSRunString(pProc)(pinstance,str,user_errors,pexit_code);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_run_file')
  else Result:=TGSRunFile(pProc)(pinstance,file_name,user_errors,pexit_code);
end;
...
begin
  ...
  if (pProc = nil) then GSAPIError('gsapi_exit')
  else Result:=TGSExit(pProc)(pinstance);
end;
