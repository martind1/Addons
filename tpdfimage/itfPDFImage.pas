...
unit itfPDFImage;
(*
06.03.14 md  D7 -> DXE2 mit Unicode
*)
...
Type
  ...
  private
    FCallback:TDisplayCallback; //structure for declaring callbacks from gsdll32.dll
    ...
    function GetFontPath:Ansistring;
  ...
  protected
    ...
    procedure SavePages(AStream:TStream;APageFrom,APageTo:Integer;AAppend:Boolean);
    {$endif}
  public
    ...
    {$ifdef USESYNPDF}
    procedure ExtractPagesToFile(AFileName:String;APageFrom,APageTo:Integer;AAppend:Boolean);
    procedure ExtractPagesToStream(AStream:TStream;APageFrom,APageTo:Integer;AAppend:Boolean);
    {$endif}
...
Var
  ...
  PathToGSDLL:AnsiString;
  PathToGSLib:AnsiString;
  PathToGSFonts:AnsiString;
...
implementation
  ...
  begin
    if FInCount or FinCheck then FInternalPageCount:=0;
  ...
  function display_presize(handle:Pointer;device:Pointer;width:integer;height:integer;raster:integer;format:UINT):integer;cdecl;
  begin
    result:=0;
  end;

  function display_size(handle:Pointer;device:Pointer;width:integer;height:integer;raster:integer;format:UINT;pimage:PAnsiChar):integer;cdecl;
  begin
    if not FInCount and not FInCheck then image_size(width,height,raster,format,pimage);
    result:=0;
  end;
  ...
  // for debugging
  //----------------------------------------------------------
  function gsstdin(caller_handle:Pointer;buf:PAnsiChar;len:integer):integer;stdcall;
  var S:String;
  begin
    S:=buf;
    FStdOut:=FStdOut + S;
  ...
  function gsstdout(caller_handle:Pointer;buf:PAnsiChar;len:integer):integer;stdcall;
  var S:String;
  begin
    S:=buf;
    setlength(S,len);
  ...
  function gsstderr(caller_handle:Pointer;buf:PAnsiChar;len:integer):integer;stdcall;
  var S:String;
  begin
    S:=buf;
    setlength(S,len);
  ...
  begin
    ...
    if Assigned(FAppendImage) then FAppendImage.Free;
    //Dispose(FInstance);
    Inherited;
  end;
  ...
  begin
    ...
    begin
      ...
      //MD<< entfernt
      //if FileExists(TPDFImage(Source).FTempFile) then LoadFromFile(TPDFImage(Source).FTempFile);
      //>>MD
  ...
  begin
    ...
    if (lResult <> 0) then raise Exception.Create('Impossible to open an instance of ghostscript. Error code: '+IntToStr(lResult));

    lResult:=GSAPISetStdio(hDLLApi,FInstance,@gsstdin,@gsstdout,@gsstderr);
    if lResult < 0 then raise Exception.Create('Impossible to assign callback stdio functions: '+IntToStr(lResult));
  end;
  ...
  begin
    ...
    begin
      if (FPageCount > 0) then FCurrentPage:=Max(1,Min(FPageCount,AValue))
      else FCurrentPage:=Max(1,AValue); // for preloading the startpage
      if (FPageCount > 0) then LoadPage(FTempFile,FCurrentPage);
    end;
  end;
  ...
  begin
    ...
    ABitmap.Dormant;
    ABitmap.FreeImage;
    if MultiThreaded then DeleteObject(ABitmap.ReleaseHandle);
    {$endif}
  end;
  ...
  function TPDFImage.GetFontPath:AnsiString;
  var PIDL: PItemIDList;
      Path: LPSTR;
      AMalloc: IMalloc;
  begin
    Path := AnsiStrAlloc(MAX_PATH);  //MD
    SHGetSpecialFolderLocation(Application.Handle, CSIDL_FONTS,PIDL);
    if SHGetPathFromIDListA(PIDL, Path) then Result := Path;  //MD
    SHGetMalloc(AMalloc);
    AMalloc.Free(PIDL);
    StrDispose(Path);
  end;
  ...
  begin
    ...
    Try
      ...
      Try
        gsOpen;
        Try
          lFormat:=DISPLAY_COLORS_NATIVE or DISPLAY_ALPHA_NONE or DISPLAY_DEPTH_1 or DISPLAY_LITTLEENDIAN or DISPLAY_BOTTOMFIRST;

          SetLength(argv,8);
          argv[0]:=PAnsiChar(AnsiString(ParamStr(0)));
          argv[1]:=PAnsiChar('-dNOPAUSE');
          argv[2]:=PAnsiChar('-dBATCH');
          argv[3]:=PAnsiChar('-sDEVICE='+'display');
          S4 := '-dDisplayFormat='+IntToStr(lFormat);
                argv[4]:=PAnsiChar(S4);
          S5 := '-r'+IntToStr(2);        //minimum resolution (for speed reasons)
                argv[5]:= PAnsiChar(S5);
          argv[6]:=PAnsiChar('-I' + PathToGSLib + ';' + PathToGSFonts + ';' + GetModulePath);
          argv[7]:=PAnsiChar(AnsiString(AFileName));

          FStdErr:='';
          FStdOut:='';
          lResult:=GSAPISetDisplayCallback(hDLLApi,FInstance,@Fcallback);
          if (lResult <> 0) then raise Exception.Create('Error while setting display callback : ' + IntToStr(lResult));

          lResult:=GSAPIInitWithArgs(hDLLApi,FInstance,length(argv),argv);
          if (lResult <> 0) then Raise Exception.Create('Error from ghostscript while convertinf file ' + AFileName + ' : Code ' + IntToStr(lResult) + ' Error: ' + FStdErr);
        finally
          gsExit;
          gsClose;
        end;
    ...
    Try
      ...
      Try
        ...
        Try
          ...
          S5 := '-r'+IntToStr(2);        //minimum resolution (for speed reasons)
                argv[5]:= PAnsiChar(S5);
          S6 := '-dFirstPage=' + IntToStr(APage);
                argv[6]:=PAnsiChar(S6);
          S7 := '-dLastPage=' + IntToStr(APage);
                argv[7]:= PAnsiChar(S7);
          argv[8]:=PAnsiChar('-I' + PathToGSLib + ';' + PathToGSFonts + ';' + GetModulePath);
          argv[9]:=PAnsiChar(AnsiString(AFileName));
  ...
  begin
    ...
    if not FileExists(FTempFile) then exit;
    ...
    Try
      gsOpen;
      Try
        FFormat:= DISPLAY_COLORS_RGB or DISPLAY_ALPHA_NONE or DISPLAY_DEPTH_8 or DISPLAY_LITTLEENDIAN or DISPLAY_BOTTOMFIRST;
        ...
        argv[2]:=PAnsiChar('-dBATCH');
        argv[3]:=PAnsiChar('-sDEVICE='+'display');
        S4 := '-dDisplayFormat='+IntToStr(Fformat);
              argv[4]:=PAnsiChar(S4);
        S5 := '-r'+IntToStr(Round(FResolution*FZoom/100));
              argv[5]:= PAnsiChar(S5);
        S6 := '-dFirstPage=' + IntToStr(APage);
              argv[6]:=PAnsiChar(S6);
        S7 := '-dLastPage=' + IntToStr(APage);
        ...
        argv[8]:=PAnsiChar('-dTextAlphaBits=4');
        argv[9]:=PAnsiChar('-sFONTPATH=' + GetFontPath);
        argv[10]:=PAnsiChar('-I' + PathToGSLib + ';' + PathToGSFonts + ';' + GetModulePath);
        argv[11]:=PAnsiChar(AnsiString(AFileName));
        ...
        lResult:=GSAPIInitWithArgs(hDLLApi,FInstance,length(argv),argv);
        if (lResult <> 0) then raise Exception.Create('Error while Converting File ' + AFileName + ' : ' + IntToStr(lResult) + ' Error: ' + FStdErr);
  ...
  Var FCurrent,FJumpWidth,FCount:Integer;
  ...
  begin
    ...
    Repeat
      ...
      begin
        if (FJumpwidth > 1) or FFound then FJumpWidth:=FJumpWidth Div 2;
        if not FMaxFound and (FJumpWidth > 1) then FJumpWidth:=FJumpWidth Div 2;
        FCurrent:=FCurrent - FJumpWidth;
        FMaxFound:=TRUE;
      end;
  ...
  begin
    ...
    if ProgressivePageCount then FPagecount:=CountPagesProgressive(FTempFile)
    else FPagecount:=CountPagesLinear(FTempFile);

    //Application.Messagebox(PAnsiChar ('Count 1 = ' + IntToStr(FCount1) + ' at ' + FormatDateTime('hh:nn:ss',dtTime1) + chr(13) + chr(10) + 'Count 2 = ' + IntToStr(FCount2) + ' at ' + FormatDateTime('hh:nn:ss',dtTime2)),'Debug',MB_OK);
    if (Pagecount > 0) then
    begin
  ...
  begin
    ...
    APDFDoc.Canvas.DrawXObject(0,0,FPDFPage.PageWidth,FPDFPage.PageHeight,'image'+inttostr(FPDFPage.ObjectNumber));
    Self.PixelFormat:=pf24bit;
  end;

  procedure TPDFImage.SavePages(AStream:TStream;APageFrom,APageTo:Integer;AAppend:Boolean);
  Var I:Integer;
  ...
  procedure TPDFImage.ExtractPagesToFile(AFileName:String;APageFrom,APageTo:Integer;AAppend:Boolean);
  ...
  begin
    ...
    Try
      ...
      if Assigned(FAppendImage) then FreeAndNil(FAppendImage);
    end;
  end;

  procedure TPDFImage.ExtractPagesToStream(AStream:TStream;APageFrom,APageTo:Integer;AAppend:Boolean);
  begin
  ...
  begin
    if (FPageCount > 0) then FCurrentPage:=Min(FPageCount,FCurrentPage+1);
    LoadPage(FTempFile,FCurrentPage);
  end;

  procedure TPDFImage.PreviousPage;
  begin
    if (FPageCount > 0) then FCurrentPage:=Max(1,FCurrentPage-1);
    LoadPage(FTempFile,FCurrentPage);
  end;

  procedure TPDFImage.FirstPage;
  begin
    if (FPageCount > 0) then FCurrentPage:=1;
    LoadPage(FTempFile,FCurrentPage);
  end;

  procedure TPDFImage.LastPage;
  begin
    if (FPageCount > 0) then FCurrentPage:=FPageCount;
    LoadPage(FTempFile,FCurrentPage);
  end;
