...
{*******************************************************}
{
  $Header: /cvsroot/delphi/ADDONS/dxmlrpc/source/XmlRpcServer.pas,v 1.1 2011/01/25 16:39:53 cvs Exp $
  ----------------------------------------------------------------------------
...
type
  ...
  //http://conferences.embarcadero.com/hu/article/32160
  ...
  if FStack.Count > 0 then
    ...
    begin
      ...
      if (FLastTag = 'INT4') then
        TRpcParameter(FStack.Peek).AsInteger := StrToInt(Data);
      if (FLastTag = 'DOUBLE') then
        TRpcParameter(FStack.Peek).AsFloat := StrToFloatIntl(Data);
      if (FLastTag = 'DATETIME.ISO8601') then
...
begin
  ...
  AnsiData := Data;
  FParser.LoadFromBuffer(PAnsiChar(AnsiData));
  FParser.StartScan;
