...
{*******************************************************}
{
  $Header: /cvsroot/delphi/ADDONS/dxmlrpc/demos/delphi/client/introspect/main.pas,v 1.1 2011/01/25 16:39:51 cvs Exp $
  ----------------------------------------------------------------------------
...
begin
  ...
  try
    ...
    begin
      ShowMessageFmt('XML-RPC Error: (%d) %s', [RpcResult.ErrorCode,
          RpcResult.ErrorMsg]);
      Exit;
    end;
...
begin
  ...
  begin
    ...
    try
      ...
      //<<MD
      RpcFunction := TRpcFunction.Create;
      RpcFunction.ObjectMethod := lbMethod.Items[lbMethod.ItemIndex];
      RpcFunction.AddItem(memSignature.Lines[0]);
      RpcResult := RpcCaller.Execute(RpcFunction);
      if RpcResult.IsError then
      begin
        //ShowMessageFmt('Error: (%d) %s', [RpcResult.ErrorCode, RpcResult.ErrorMsg]);
        memHelp.Lines.Add(Format('Error: (%d)', [RpcResult.ErrorCode]));
        memHelp.Lines.Add(Format('%s', [RpcResult.ErrorMsg]));
        Exit;
      end;
      ...
      begin
        ...
        for I := 0 to Signatures.Count - 1 do
          memHelp.Lines.Add(Signatures[I].AsString);
      end else
