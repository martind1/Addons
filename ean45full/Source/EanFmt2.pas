...
begin
     ...
     EanMemo.Text   := String(Ean.BarCode);
     AddUp.Visible  := (EAN.TypBarCode=bcEan13);
     ZNAKY.Caption  := String(EAN.GetSetOfCharsVisible);
end;
...
begin
     ...
     for i:= Low(MyColors) to High(MyColors) do
         BGColor.Items.Add(String(MyColors[i].Name));
...
begin
  ...
  with TComboBox(Control).Canvas do begin
    ...
    StrPCopy(Text, String(MyColors[Index].Name));
...
begin
     ...
     CA_TEXT.Text            := String(E.Caption.Text);
     ...
     BCA_TEXT.Text            := String(E.CaptionBottom.Text);
...
begin
     EAN.BarCode := AnsiString(EanMemo.Text);
     EAN.Width   := EAN.MinWidth;
end;
...
begin
     ...
     ParentEAN.BarCode        := AnsiString(EanMemo.Text);
     ...
     ParentEAN.LabelMask := AnsiString(s);
