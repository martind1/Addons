...
begin
     if FEan.BarCode <> AnsiString(Value) then begin
        FEan.BarCode := AnsiString(Value);
        RekalkWidthMM;
        Invalidate;
     end;
end;
...
begin
  Result := String(FEan.GetSetOfChars);
end;

function   TQrCustomEan.GetSetOfCharsVisible:String;
begin
  Result := String(FEan.GetSetOfCharsVisible);
end;

function   TQrCustomEan.CheckBarCode(var S:String):Boolean;
begin
     Result := FEan.CheckBarCode(AnsiString(S));
end;
...
begin
  Result := String(FEan.LastPaintErrorText);
end;
...
begin
  Result := String(FEan.LabelMask);
end;


Procedure   TQRCustomEan.SetLabelMask(Value:String);
begin
  FEan.LabelMask := AnsiString(Value);
end;
