...
type
 ...
 TBarCodePaintEvent = procedure (Sender: TObject; R:TRect; ABarCode:AnsiString) of object;
 ...
 TLastPaintError = (erOK, erMustBe, erErrorInBarCode, erCharOutOfSet,
  ...
  private
         ...
         FAutoIncTo      : Integer;
         FFirstCode      : AnsiString;
         FSecurity       : Boolean;
         FTransparent    : Boolean;
         FBackGroundColor: TColor;
         FLastPaintError : TLastPaintError;
         FLastErrorIndex : Integer;
         FLastErrorChar  : AnsiChar;
         FLabelMask      : AnsiString;
         FAngle          : Integer;
         ...
         {$ifdef PSOFT_PDF417}
                FPDF417:TpsPDF417;
         {$endif}
         procedure   SetBarCode(Value:AnsiString);
         procedure   SetLabelMask(Value:AnsiString);
         ...
         function    Codabar(ck:AnsiString):AnsiString;
         function    AbcCodabar(ck:AnsiString):AnsiString;
         function    Code39Standard(ck:AnsiString):AnsiString;
         function    Code39Full(ck:AnsiString):AnsiString;
         function    Code93Standard(ck:AnsiString):AnsiString;
         function    Code93Full(ck:AnsiString):AnsiString;
         function    Code128(ck:AnsiString; T:TTypBarCode):AnsiString;
         function    EanAddOn(ck:AnsiString; le:Integer):AnsiString;
         function    Ean8(ck:AnsiString):AnsiString;
         function    Ean13(ck:AnsiString):AnsiString;
         function    UPCA(ck:AnsiString):AnsiString;
         function    UPCE(ck:AnsiString):AnsiString;
         // function    ITF14(ck:AnsiString):AnsiString;
         function    Plessey(ck:AnsiString):AnsiString;
         function    Datalogic(ck:AnsiString):AnsiString;
         function    Matrix(ck:AnsiString):AnsiString;
         function    Industrial(ck:AnsiString):AnsiString;
         function    Interleaved(ck:AnsiString):AnsiString;
         function    IATA(ck:AnsiString):AnsiString;
         function    Invert(ck:AnsiString):AnsiString;
         function    Coop(ck:AnsiString):AnsiString;
         function    Code11(ck:AnsiString):AnsiString;
         function    PostNet(ck:AnsiString):AnsiString;
         function    PZN(ck:AnsiString):AnsiString;
         function    Zoom(ABarCode:AnsiString; AWidth:integer; ASecurity:Boolean):AnsiString;
         ...
         function    CheckEAN8 (var s:AnsiString):Boolean;
         function    CheckEAN13(var s:AnsiString):Boolean;
         function    CheckUPCA (var s:AnsiString):Boolean;
         function    TestCharsInSet(s:AnsiString):Boolean;
         procedure   SetSetOfChars(Value:AnsiString);
         procedure   HorzLinesChange(Sender:TObject);
         procedure   SetAutoCheckDigit(Value:Boolean);
  protected
         procedure   Paint; override;
         function    GetBarCodeLines(var BC:AnsiString):AnsiString;
  ...
  public
         ...
         procedure   Copyright;
         procedure   ActiveSetupWindow(Sheet:AnsiString);         virtual;
         ...
         function    GetSetOfChars:AnsiString;
         function    GetSetOfCharsVisible:AnsiString;
         function    CheckBarCode(S:AnsiString):Boolean;
	...
	{$IFDEF PSOFT_JPEG}
        ...
        {$ENDIF}
         ...
         function    LastPaintErrorText:AnsiString;
         ...
         function    BarcodeTypeName:AnsiString;
         ...
         property BarCode           : AnsiString         Read FBarCode         Write SetBarCode;
         property LabelMask         : AnsiString         Read FLabelMask       Write SetLabelMask;
         property FirstBarCode      : AnsiString         Read FFirstCode       Write FFirstCode;
         ...
         property SetOfChars        : AnsiString Read GetSetOfChars Write SetSetOfChars Stored False;
  ...
  function  IsBookland(ISBN:AnsiString):Boolean;
  function  BooklandToEAN(s:AnsiString; T:TTypBarcode):AnsiString;
  function  CalcEan8CheckDigit(s:AnsiString):AnsiChar;
  function  CalcEan13CheckDigit(s:AnsiString):AnsiChar;
  function  CalcISBNCheckDigit(s:AnsiString):AnsiChar;
  function  RemoveControlChars(s:AnsiString):AnsiString;
...
//nach oben 15.05.12 {$R eankod.res}
...
{$endif}
   ...
   resourcestring
    ...
    ErrUnavailableChar     = 'Unvailable AnsiChar for this type of barcode.';
    ...
    ErrFirstCharZero       = 'ITF14 - first AnsiChar must be 0.';
    ErrCharSpace           = '. AnsiChar must be space.';
...
procedure DrawRotatedText(C:TCanvas; X,Y:Integer; rc:TPoint; A:Integer; T:AnsiString);
var rp:TPoint;
begin
     rp := RotatePoint( Point(X, Y), rc,A);
     SetTextAlign(C.Handle, TA_LEFT);
     SetBkMode(C.Handle,TRANSPARENT);
     C.TextOut(rp.X, rp.Y, String(T));
end;
...
procedure TBarcodeCaption.SetText(Value:AnsiString);
begin
...
function GetFileName(N1,N2:AnsiString):AnsiString;
var S:TSaveDialog;
begin
     Result:='';
     ...
     try
           S.Filter := String(N1);
           S.DefaultExt := String(N2);
           S.Options := S.Options + [ofOverwritePrompt];
           if S.Execute then Result := AnsiString(S.FileName);
     finally
           S.Free;
     end;
end;

function TCustomEan.TestCharsInSet(s:AnsiString):Boolean;
var i:Integer;
    mn:AnsiString;
begin
    Result:=True;
...
function TCustomEan.CheckEAN13(var s:AnsiString):Boolean;
begin
...
function TCustomEAN.CheckEan8(var s:AnsiString):Boolean;
begin
...
function TCustomEAN.CheckUPCA(var s:AnsiString):Boolean;
begin
...
function TCustomEan.GetSetOfChars:AnsiString;
begin
     result := AnsiString(BarcodeInfo(FTypBarCode).Chars);
end;

function    TCustomEan.GetSetOfCharsVisible:AnsiString;
var s:AnsiString;
    i:Integer;
begin
     s:=GetSetOfChars;
     Result:='';
     for i:=1 to Length(s) do
         if s[i]<' ' then Result:=Result+'^'+AnsiChar(Ord(s[i])+Ord('A'))+' '
         else             Result:=Result+s[i]+' ';
     s:=Result;
end;

function TCustomEan.CheckBarCode(S:AnsiString):Boolean;
var pom : AnsiString;
    i   : Integer;
begin
     result := (s<>'');
...
function TCustomEan.EanAddOn(ck:AnsiString; le:Integer):AnsiString;
var i,j :Integer;
    s,sp:AnsiString;
begin
     // s     := '       ';
     ...
     if Length(ck)=2 then begin
        ...
        case sp[1] of
          'A': S := S + AnsiString(Copy(String(ean_sa), 8*(Ord(ck[1])-Ord('0'))+1, 7));
          'B': S := S + AnsiString(Copy(String(ean_sb), 8*(Ord(ck[1])-Ord('0'))+1, 7));
        end;
        s:=s+'01';
        case sp[2] of
          'A': S := S + AnsiString(Copy(String(ean_sa), 8*(Ord(ck[2])-Ord('0'))+1, 7));
          'B': S := S + AnsiString(Copy(String(ean_sb), 8*(Ord(ck[2])-Ord('0'))+1,7));
        end;
     end;
     ...
     if Length(ck)=5 then begin
        ...
        for i:=1 to Length(ck) do begin
            if i>1 then s:=s+'01';
            case sp[i] of
                'A': S := S + AnsiString(Copy(String(ean_sa), 8*(Ord(ck[i])-Ord('0'))+1,7));
                'B': S := S + AnsiString(Copy(String(ean_sb), 8*(Ord(ck[i])-Ord('0'))+1,7));
                'C': S := S + AnsiString(Copy(String(ean_sc), 8*(Ord(ck[i])-Ord('0'))+1,7));
            end;
        end;
     end;
...
function TCustomEan.Ean8(ck:AnsiString):AnsiString;
var i,ma : integer;
    s    : AnsiString;
begin
   ma        := Length(ck);
   ...
   s:='202';
   H1:=Length(s)+1;
   for i:=1 to 4 do S := S + AnsiString(Copy(String(ean_sa), 8*(Ord(ck[i])-Ord('0'))+1, 7));
   H2:=Length(s)-1;
   s:=s+'02020';
   H3:=Length(s)+1;
   for i:=5 to 8 do
         S := S + AnsiString(Copy(String(ean_sc), 8*(Ord(ck[i])-Ord('0'))+1,7));
   H4:=Length(s)-1;
   s:=s+'202';
...
function TCustomEan.Ean13(ck:AnsiString):AnsiString;
var i             : integer;
    s1            : AnsiChar;
    s,sp          : AnsiString;
begin
   ...
   sp := AnsiString(' ' + Copy(String(ean_sm), 7*(Ord(ck[1])-Ord('0'))+1, 6));
   s:='202';
   H1:=Length(s)+1;
   for i:=2 to 7 do begin
       s1:=sp[i];
       case s1 of
         'A': S := S + AnsiCopy(ean_sa,8*(Ord(ck[i])-Ord('0'))+1,7);
         'B': S:=S+AnsiCopy(ean_sb,8*(Ord(ck[i])-Ord('0'))+1,7);
         'C': S:=S+AnsiCopy(ean_sc,8*(Ord(ck[i])-Ord('0'))+1,7);
       end;
   end;
   ...
   s:=s+'02020';
   H3:=Length(s)+1;
   for i:=8 to 13 do S:=S+AnsiCopy(ean_sc,8*(Ord(ck[i])-Ord('0'))+1,7);
   H4:=Length(s)-1;
   s:=s+'202';

   Result:=s;
   if length(ck)>13 then
      Result:=Result+EanAddOn(AnsiCopy(ck,15,5), Length(Result));
end;

function TCustomEan.UPCA(ck:AnsiString):AnsiString;
var i:Integer;
    s:AnsiString;
begin
     { start sequence}
     ...
     for i:=1 to 6 do
         S:=S+AnsiCopy(ean_sa,8*(Ord(ck[i])-Ord('0'))+1,7);
     ...
     for i:=7 to 12 do
         S:=S+AnsiCopy(ean_sc,8*(Ord(ck[i])-Ord('0'))+1,7);
     ...
     if length(ck)>12 then
        Result:=Result+EanAddOn(AnsiCopy(ck,14,5), Length(Result));

end;

function TCustomEan.UPCE(ck:AnsiString):AnsiString;
...
begin
     ...
     for i:=2 to 7 do begin
         ...
         if P='O' then S:=S+AnsiCopy(ean_sa,8*(Ord(ck[i])-Ord('0'))+1,7)
         else          S:=S+AnsiCopy(ean_sb,8*(Ord(ck[i])-Ord('0'))+1,7);
     end;
     H2 := Length(s)-1;
     s  := s+'020202';
     H6 := Length(s)+1;
     ...
     { UPC with extension ?}
     if length(ck)>8 then
        Result:=Result+EanAddOn(AnsiCopy(ck,10,5), Length(Result));
end;

function    TCustomEan.Plessey(ck:AnsiString):AnsiString;
...
var s,pom,pom1  : AnsiString;
    i,j,su      : Integer;
    function MSICheck(ck:AnsiString):AnsiChar;
    var su_e,i : Integer;
        pom    : AnsiString;
    begin
         su := 0;
...
function  TCustomEan.PostNet(ck:AnsiString):AnsiString;
...
begin
     ...
     cs:=10-(cs mod 10);
     if cs=10 then cs:=0;
     p :=p+PostnetTable[AnsiChar(cs+Ord('0'))]+'1';
     ...
     result:=AnsiCopy(s,1,Length(s)-1);
end;


function TCustomEan.Codabar(ck:AnsiString):AnsiString;
var s   :AnsiString;
    i   :integer;
    suma:integer;
    mn  :AnsiString;
...
function TCustomEan.Code39Standard(ck:AnsiString):AnsiString;
var s:AnsiString;
    i:integer;
begin
...
function TCustomEan.Code39Full(ck:AnsiString):AnsiString;
var s:AnsiString;
    I:INTEGER;
begin
     s:='*+$*';
...
function TCustomEan.Datalogic(ck:AnsiString):AnsiString;
var s:AnsiString;
    i:integer;
begin
     Result:='2020';
...
function TCustomEan.Matrix(ck:AnsiString):AnsiString;
var s:AnsiString;
    i:integer;
begin
     Result:='22202020';
...
function TCustomEan.Coop(ck:AnsiString):AnsiString;
var s:AnsiString;
    i:integer;
begin
     Result:='220220';
...
function TCustomEan.Code11(ck:AnsiString):AnsiString;
var s,m   : AnsiString;
    i,j,k : integer;
begin
...
function TCustomEan.Industrial(ck:AnsiString):AnsiString;
var s:AnsiString;
    i:integer;
begin
     Result:='22022020';
...
function TCustomEan.Interleaved(ck:AnsiString):AnsiString;
const def   : AnsiString='00110 10001 01001 11000 00101 10100 01100 00011 10010 01010';
      _space: AnsiString='          ';
var s     : AnsiString;
    i,j   : integer;
    s1,s2 : Byte;
...
function TCustomEan.Code93Standard(ck:AnsiString):AnsiString;
var i,j,k : integer;
    s     : AnsiString;
    suma  : integer;
    mn    : AnsiString;
...
function TCustomEan.Code93Full(ck:AnsiString):AnsiString;
var i:integer;
    s:AnsiString;
begin
     Result:=c93_start;
...
function TCustomEan.Code128(ck:AnsiString; T:TTypBarCode):AnsiString;
const CODE128_STOP_ALL = '2200022202022';
    ...
    Code128Table: array[0..105] of String[11] = (
        ...
        '22020000200', '22020020000', '22020022200');

        function IsControl(c:AnsiChar):Boolean;
        begin
             result := (c in [#0..#31]);
        end;

        function Have4OrMoreDigits(const s:AnsiString; index:integer):integer;
        var i:integer;
        begin
             result := 0;
             i      := index;
        ...
        function LowerAfterControl(const s:AnsiString; index:integer):Boolean;
        begin
             Result:=False;
        ...
        function ControlLowerControl(const s:AnsiString; index:integer):Boolean;
        begin
             Result:=False;
        ...
        function Compress128(s:AnsiString):AnsiString;
        var i,j:integer;
            AktTyp:AnsiChar;
            temp:AnsiString;

            procedure SwitchToCharSet(c:AnsiChar);
            begin
                 case c of
                   #0..#31 : case AktTyp of
                                'A' : ;
                                'B' : temp := temp+Code128Shift;
                                'C' : begin
                                           temp := temp+Code128CodeB+Code128Shift;
                                           AktTyp := 'B';
                                      end;
                             end;
                   ' '..'_' : case AktTyp of
            ...
            function ConvertChar(c:AnsiChar):AnsiChar;
            begin
                 if c in [#0..#31] then Result := AnsiChar(Chr(64+Ord(c)))
                 else                   Result := AnsiChar(Chr(Ord(c)-32));
            end;
        begin
             Result:='';
             if s='' then Exit;
             ...
             if j>=4 then begin
                  ...
                  while j>0 do begin
                          Result:=result+AnsiChar( StrToInt(String(AnsiCopy(s,i,2))) );
                          Inc(i,2);
                          Dec(j,2);
                  end;
                  if j=1 then begin
                     SwitchToCharSet(s[i]);
                     Result:=Result+ConvertChar(s[i]);
                     Inc(i);
                  end;
                 ...
                 if j>=4 then begin
                    ...
                    if AktTyp<>'C' then begin
                       if (j mod 2)=0 then
                          ...
                          SwitchToCharSet(s[i]);
                          Result:=Result+ConvertChar(s[i])+Code128CodeC;
                          Inc(i);
                          Dec(j);
                       end;
                       AktTyp:='C';
...
begin
     ...
     tmp:=tmp+AnsiChar(j mod 103);

     for i:=1 to Length(tmp) do
         result:=result+Code128Table[Ord(tmp[i])];
...
function TCustomEan.AbcCodabar(ck:AnsiString):AnsiString;
var s:AnsiString;
    i:integer;
begin
     for i:=1 to Length(ck) do begin
...
function TCustomEan.Iata(ck:AnsiString):AnsiString;
var s:AnsiString;
    i:integer;
begin
     Result:='2020';
...
function TCustomEan.Invert(ck:AnsiString):AnsiString;
var s:AnsiString;
    i:integer;
begin
     Result:='20002000202';
...
function  TCustomEan.PZN(ck:AnsiString):AnsiString;
var s:AnsiString;
begin
    s:='-'+ck;
    Result := Code39Standard(s);
...
function TCustomEan.GetBarCodeLines(var BC:AnsiString):AnsiString;
var s,pr : AnsiString;
    i    : Integer;
    procedure MoveRight;
    begin
...
begin
     ...
     case FTypBarCode of
        ...
        bcUPCShipping    : begin
                         i := 0;
                         if AnsiCopy(Barcode,16,6)<>'' then begin
                            s := Interleaved(AnsiCopy(Barcode,16,6));
                            s[1]:='X';
                            for i:=1 to Length(s) do
                                if s[i]='2' then s[i]:='1';
                            i:=Length(s);
                         ...
                         s  := Interleaved(AnsiCopy(Barcode,1,14)) + s;
                         H5 := Length(s) - i + 20;
                    end;
...
begin
    ...
    s:=AnsiCopy(s,ai_from,ai_to-ai_from+1);
    ...
    s:=AnsiCopy(BarCode,1,ai_from-1)
      +s
      +AnsiCopy(BarCode,ai_to+1,1000);

    case TypBarCode of
         bcEan8  : s:=AnsiCopy(s,1,7);
         bcEan13 : s:=AnsiCopy(s,1,12);
    end;

    if CheckBarCode(s) then BarCode:=s;
end;

procedure   TCustomEan.SetBarCode(Value:AnsiString);
begin
...
procedure   TCustomEan.SetLabelMask(Value:AnsiString);
begin
...
begin
     if FTypbarCode<>Value then begin
        ...
        BarCode     := AnsiString(BI.InitValue);
...
function TCustomEan.Zoom(ABarCode:AnsiString; AWidth:integer; ASecurity:Boolean):AnsiString;
...
procedure PaintBarCode1(C:TCanvas; R:TRect; E:TCustomEan; ABarCode:AnsiString; RC:TPoint;
...
var Y,i,j      : integer;
    ...
    FontHeight : integer;
    AAddUp     : Boolean;
    BarCodeLabel : AnsiString;
    W,_lc        : Integer;
    R1           : TRect;
    P            : TPen;
    procedure DrawEanText(T1,T2,T3:AnsiString);
    var i:Integer;
    begin
...
begin
     ...
     with C do begin
          ...
          if E.FontAutoSize then begin
             case E.TypBarCode of
                bcEan8,bcJAN8                   : i:=TextWidth(AnsiCopy(ABarCode,1,4));
                bcEan13, bcISBN, bcISSN, bcISMN,
                bcUpcE0,bcUpcE1,bcJAN13         : i:=TextWidth(AnsiCopy(ABarCode,2,6));
                bcUPCA                          : i:=TextWidth(AnsiCopy(ABarCode,2,5));
          ...
          if E.ShowLabels then begin
             ...
             for i:=1 to Length(BarCodeLabel) do
                 if (AnsiCopy(s,i,1)='') or (AnsiCopy(s,i,1)='_') then begin end
             ...
             case
                 E.TypBarCode of
                      bcEan8,bcJAN8  : DrawEanText(AnsiCopy(BarCodeLabel,1,4),
                                                   AnsiCopy(BarCodeLabel,5,4),
                                                   AnsiCopy(BarCodeLabel,10,5));
                      bcEan13,bcISBN, bcISSN, bcISMN,bcJAN13 : begin
                                     s:=BarCodeLabel;
                                     DrawRotatedText(C, R.Left, Y, rc,E.Angle, BarCodeLabel[1]);
                                     DrawEanText(AnsiCopy(BarCodeLabel,2,6),
                                                 AnsiCopy(BarCodeLabel,8,6),
                                                 AnsiCopy(BarCodeLabel,15,5));
                                end;
                      bcUPCA : begin
                                     s:=BarCodeLabel;
                                     DrawRotatedText(C, R.Left, Y, rc,E.Angle, BarCodeLabel[1]);
                                     DrawEanText(AnsiCopy(BarCodeLabel,2,5),
                                                 AnsiCopy(BarCodeLabel,7,5),
                                                 AnsiCopy(BarCodeLabel,14,5));
                      ...
                      bcUPCShipping : DrawEanText(AnsiCopy(ABarCode,1,1)
                                               +'  '+AnsiCopy(ABarCode, 2,2)
                                               +'  '+AnsiCopy(ABarCode, 4,5)
                                               +'  '+AnsiCopy(ABarCode, 9,5)
                                               +'  '+AnsiCopy(ABarCode,14,1),
                                               '',
                                               AnsiCopy(ABarCode,16,6));
                      bcPostNet : ;
                      bcOPC : begin
                                       s:=AnsiCopy(BarCodeLabel, 1,5)+'-'
                                         +AnsiCopy(BarCodeLabel, 6,4)+'-'
                                         +AnsiCopy(BarCodeLabel,10,1);
                                       i:=H2-H1-TextWidth(s);
...
procedure DrawLines(C:TCanvas; R:TRect; const s:AnsiString);
var i,x1,x2:Integer;
begin
        i  := 1;
        x1 := R.Left;
...
function    TCustomEan.LastPaintErrorText:AnsiString;
begin
     Result := AnsiString(ErrorsText[FLastPaintError]);
     if FLastPaintError=erCharOutOfSet then
        Result:=Result+Format('(%d.AnsiChar - %s)',[FLastErrorIndex, FLastErrorChar]);
...
procedure TCustomEAN.ActiveSetupWindow(Sheet:AnsiString);
var i:Integer;
begin
...
begin
    ...
    if j<10 then Result:=AnsiChar(j+Ord('0'))
    else         Result:='X';
end;
...
begin
     ...
     if Length(Result)>=13 then
        Result[13]:=CalcEan13CheckDigit(AnsiCopy(Result,1,12));
end;


function  CheckDigit212Mod10(s:AnsiString):AnsiChar;
var i,su,na :Integer;
begin
    su:=0;
    for i:=1 to Length(s) do begin
     ...
     case FTypBarCode of
          bcEan8,bcJAN8  : begin
                         if FTypBarCode=bcJAN8 then
                            s:='49'+AnsiCopy(s,3,20);
                         if Length(s)=7 then s:=s+' ';
                         if Length(s)>=8 then
                            s[8] := CalcEan8CheckDigit(AnsiCopy(s,1,7));
                    end;
          bcEan13,bcJAN13 : begin
                         ...
                         if Length(s)=12 then s:=s+' ';
                         if Length(s)>=13 then
                            s[13] := CalcEan13CheckDigit(AnsiCopy(s,1,12));
                    end;
          bcUPCA : begin
                         if Length(s)=11 then s:=s+' ';
                         if Length(s)>=12 then
                            s[12] := CalcEan8CheckDigit(AnsiCopy(s,1,11));
                    end;
          bcITF  : begin
                         if Length(s) in [5,13,15] then s:=s+' ';
                         case Length(s) of
                              6  : s[ 6] := CalcEan8CheckDigit(AnsiCopy(s,1, 5));
                              14 : s[14] := CalcEan8CheckDigit(AnsiCopy(s,1,13));
                              16 : s[16] := CalcEan8CheckDigit(AnsiCopy(s,1,15));
                         end;
                    end;
          bcISBN, bcISSN, bcISMN  : begin
                         ...
                         if Length(s)=12 then s:=s+' ';
                         if Length(s)>=13 then
                            s[13] := CalcISBNCheckDigit(AnsiCopy(s,1,12));
                    end;
          bcUPCE0,bcUPCE1 : begin
                         ...
                         if Length(s)>=8 then begin
                            i:=Ord(s[7])-Ord('0');
                            case i of
                                 0,1,2 : p:=AnsiCopy(s,1,3)+s[7]+'0000'+AnsiCopy(s,4,3);
                                 3     : p:=AnsiCopy(s,1,4)+'00000'+AnsiCopy(s,5,2);
                                 4     : p:=AnsiCopy(s,1,5)+'00000'+AnsiCopy(s,6,1);
                                 5 ..9 : p:=AnsiCopy(s,1,6)+'0000' +s[7];
                            end;
          ...
          bcUPCShipping : begin
                               ...
                               if Length(s)>=14 then
                                  s[14] := CalcEan8CheckDigit(AnsiCopy(s,1,13));
                          end;
          bcOPC         : begin
                               if Length(s)=9 then s:=s+' ';
                               if Length(s)>=10 then
                                  s[10] := CheckDigit212Mod10(AnsiCopy(s,1,9));
                          end;
          bcPZN        : begin
...
begin
     ...
     try
        ...
        L.Add('<HR>');
        L.Add('<CENTER>&AnsiCopy; PSOFT 1999, ');
...
function    TCustomEAN.BarcodeTypeName:AnsiString;
var P:PPropInfo;
begin
...
function IsBookland(ISBN: AnsiString): Boolean;
var Number, CheckDigit        : AnsiString;
    CheckValue, CheckSum, Err : Integer;
    i, Cnt                    : Word;
begin
  {Get check digit}
  CheckDigit := AnsiCopy(ISBN, Length(ISBN), 1);

  {Get rest of ISBN, minus check digit and its hyphen}
  Number := AnsiCopy(ISBN, 1, Length(ISBN) - 2);
...
procedure   TCustomEan.SetSetOfChars(Value:AnsiString);
begin
end;
