...
25.11.13 md  VertScrollbar Position nicht mehr = 0 setzen (dafür Top iAv Position)
             fixing Legende: Titel nicht neben Group Checkboxen.
13.04.14 md  Axis.Onchange fires when Pan changed
             Axis.MinSave/MaxSave public readonly
...
type
  ...
  private
    ...
    MainTicksText: array of TMainTicksText;
    procedure SetPan(const Value: Integer); //BG260906 XAchse beliebig beschriften
  ...
  public
    ...
    property MinSave: TFloat read FMinSave;
    property MaxSave: TFloat read FMaxSave;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
...
procedure TAxis.SetPan(const Value: Integer);
begin
  FPan := Value;
  if Assigned(FOnChange) then FOnChange(Self);
end;
...
begin
  ...
  sbCurveTitles.VertScrollBar.Tracking := true;
...
//MD25.11.13 test      sbCurveTitles.VertScrollBar.Position := 0; //das verhindert dass Phantom-ChkBoxen nach unten "erfunden" werden
      ...
      begin
        ...
        Width  := 15;
        Left   := x4;
        Top    := y3 - (Height div 2) - 5 - sbCurveTitles.VertScrollBar.Position;  //MD25.11.13
      end;
    end;
    inc(iPos);
  end;
...
var
  ...
  begin
    ...
    y3 := YB1 +
          (2 * ciCurveTitlesRandTop) +
          (ciCurveTitelsHeader) +
          (iPos  * ciCurveTitlesHeight) +4; //- 3    MD25.11.13 LEB
    PBCurveTitles.Canvas.Pen.Width := 1;
    PBCurveTitles.Canvas.Brush.Color := sbCurveTitles.Color; //Hintergrund normal malen
    PBCurveTitles.Canvas.Font.Style := PBCurveTitles.Canvas.Font.Style + [fsBold];
    if FCurve <> nil then
      PBCurveTitles.Canvas.TextOut(x4 + ciCurveTitlesRandLeft,
        y3 - (PBCurveTitles.Canvas.TextHeight(FCurve.Name) div 2) - 8,
        FCurveGroup.Name);
...
begin
  FXAxis.FZoom:=1.0;
  FYAxis.FZoom:=1.0;
  FXAxis.Pan := 0;  //MD13.04.14 Pan statt FPan
  FYAxis.Pan := 0;
