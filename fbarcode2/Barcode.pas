...
begin
  checksum := 0;
  {Startcode}
  result := tabelle_39[FindIdx('*')].data;//test  + '0';
  result := result + '0';
...
function Find_Code128C(c:string):integer;
  ...
  begin
    for i:=0 to High(tabelle_128) do begin
      if String(tabelle_128[i].c) = c then begin
       result := i;
       exit;
      end;
    end;
...
begin
  ...
  case FTyp of
    ...
    bcCodeEAN128C:
      ...
      result := result + String(tabelle_128[102].data);
  ...
  begin
    ...
    begin
      ...
      result := result + String(tabelle_128[idx].data);
  ...
  result := result + String(tabelle_128[checksum].data);
...
var
  i, idx : integer;
begin
  result := tabelle_cb[Find_Codabar('A')].data;  //md + '0';
  result := result  + '0';
  for i:=1 to Length(FText) do
  begin
...
begin
  //MD 26.08.10
  RegisterComponents('Addons', [TAsBarcode]);
end;
