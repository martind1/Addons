(* Pascal/Delphi runtime linkage constants and function definitions for LS17.DLL *)
...
function  LS17xModuleName: String;
function  LS17xLoad: integer;
procedure LS17xUnload;

implementation

var hDLLLS17: longint = 0;
var nDLLLS17Usage: longint = 0;
...
begin
  Result := 0;
  inc(nDLLLS17Usage);
  if (nDLLLS17Usage = 1) then
    begin
    nError := SetErrorMode($8000); { SEM_NOOPENFILEERRORBOX }
    {$ifdef UNICODESTRING_AWARE}
      hDLLLS17 := LoadLibrary(pWideChar(LS17xModuleName()));
     {$else}
      hDLLLS17 := LoadLibrary(pChar(LS17xModuleName()));
    {$endif}
    SetErrorMode(nError);
    ...
    if (hDLLLS17 = 0) then
      ...
      begin
      {$ifdef UNICODE}
          @LlStgsysStorageOpenO := GetProcAddress(hDLLLS17,'LlStgsysStorageOpenA');
        {$else}
          @LlStgsysStorageOpen := GetProcAddress(hDLLLS17,'LlStgsysStorageOpenA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysStorageOpen := GetProcAddress(hDLLLS17,'LlStgsysStorageOpenW');
        {$else}
          @LlStgsysStorageOpenO := GetProcAddress(hDLLLS17,'LlStgsysStorageOpenW');
      {$endif}
      @LlStgsysStorageClose := GetProcAddress(hDLLLS17,'LlStgsysStorageClose');
      @LlStgsysGetAPIVersion := GetProcAddress(hDLLLS17,'LlStgsysGetAPIVersion');
      @LlStgsysGetFileVersion := GetProcAddress(hDLLLS17,'LlStgsysGetFileVersion');
      {$ifdef UNICODE}
          @LlStgsysGetFilenameO := GetProcAddress(hDLLLS17,'LlStgsysGetFilenameA');
        {$else}
          @LlStgsysGetFilename := GetProcAddress(hDLLLS17,'LlStgsysGetFilenameA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysGetFilename := GetProcAddress(hDLLLS17,'LlStgsysGetFilenameW');
        {$else}
          @LlStgsysGetFilenameO := GetProcAddress(hDLLLS17,'LlStgsysGetFilenameW');
      {$endif}
      @LlStgsysGetJobCount  := GetProcAddress(hDLLLS17,'LlStgsysGetJobCount');
      @LlStgsysSetJob       := GetProcAddress(hDLLLS17,'LlStgsysSetJob');
      @LlStgsysGetJob       := GetProcAddress(hDLLLS17,'LlStgsysGetJob');
      @LlStgsysGetPageCount := GetProcAddress(hDLLLS17,'LlStgsysGetPageCount');
      @LlStgsysGetJobOptionValue := GetProcAddress(hDLLLS17,'LlStgsysGetJobOptionValue');
      @LlStgsysGetPageOptionValue := GetProcAddress(hDLLLS17,'LlStgsysGetPageOptionValue');
      {$ifdef UNICODE}
          @LlStgsysGetPageOptionStringO := GetProcAddress(hDLLLS17,'LlStgsysGetPageOptionStringA');
        {$else}
          @LlStgsysGetPageOptionString := GetProcAddress(hDLLLS17,'LlStgsysGetPageOptionStringA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysGetPageOptionString := GetProcAddress(hDLLLS17,'LlStgsysGetPageOptionStringW');
        {$else}
          @LlStgsysGetPageOptionStringO := GetProcAddress(hDLLLS17,'LlStgsysGetPageOptionStringW');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysSetPageOptionStringO := GetProcAddress(hDLLLS17,'LlStgsysSetPageOptionStringA');
        {$else}
          @LlStgsysSetPageOptionString := GetProcAddress(hDLLLS17,'LlStgsysSetPageOptionStringA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysSetPageOptionString := GetProcAddress(hDLLLS17,'LlStgsysSetPageOptionStringW');
        {$else}
          @LlStgsysSetPageOptionStringO := GetProcAddress(hDLLLS17,'LlStgsysSetPageOptionStringW');
      {$endif}
      @LlStgsysAppend       := GetProcAddress(hDLLLS17,'LlStgsysAppend');
      @LlStgsysDeleteJob    := GetProcAddress(hDLLLS17,'LlStgsysDeleteJob');
      @LlStgsysDeletePage   := GetProcAddress(hDLLLS17,'LlStgsysDeletePage');
      @LlStgsysGetPageMetafile := GetProcAddress(hDLLLS17,'LlStgsysGetPageMetafile');
      @LlStgsysDestroyMetafile := GetProcAddress(hDLLLS17,'LlStgsysDestroyMetafile');
      @LlStgsysDrawPage     := GetProcAddress(hDLLLS17,'LlStgsysDrawPage');
      @LlStgsysGetLastError := GetProcAddress(hDLLLS17,'LlStgsysGetLastError');
      @LlStgsysDeleteFiles  := GetProcAddress(hDLLLS17,'LlStgsysDeleteFiles');
      {$ifdef UNICODE}
          @LlStgsysPrintO := GetProcAddress(hDLLLS17,'LlStgsysPrintA');
        {$else}
          @LlStgsysPrint := GetProcAddress(hDLLLS17,'LlStgsysPrintA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysPrint := GetProcAddress(hDLLLS17,'LlStgsysPrintW');
        {$else}
          @LlStgsysPrintO := GetProcAddress(hDLLLS17,'LlStgsysPrintW');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysStoragePrintO := GetProcAddress(hDLLLS17,'LlStgsysStoragePrintA');
        {$else}
          @LlStgsysStoragePrint := GetProcAddress(hDLLLS17,'LlStgsysStoragePrintA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysStoragePrint := GetProcAddress(hDLLLS17,'LlStgsysStoragePrintW');
        {$else}
          @LlStgsysStoragePrintO := GetProcAddress(hDLLLS17,'LlStgsysStoragePrintW');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysGetPagePrinterO := GetProcAddress(hDLLLS17,'LlStgsysGetPagePrinterA');
        {$else}
          @LlStgsysGetPagePrinter := GetProcAddress(hDLLLS17,'LlStgsysGetPagePrinterA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysGetPagePrinter := GetProcAddress(hDLLLS17,'LlStgsysGetPagePrinterW');
        {$else}
          @LlStgsysGetPagePrinterO := GetProcAddress(hDLLLS17,'LlStgsysGetPagePrinterW');
      {$endif}
      @LsSetDebug           := GetProcAddress(hDLLLS17,'LsSetDebug');
      {$ifdef UNICODE}
          @LsGetViewerControlClassNameO := GetProcAddress(hDLLLS17,'LsGetViewerControlClassNameA');
        {$else}
          @LsGetViewerControlClassName := GetProcAddress(hDLLLS17,'LsGetViewerControlClassNameA');
      {$endif}
      {$ifdef UNICODE}
          @LsGetViewerControlClassName := GetProcAddress(hDLLLS17,'LsGetViewerControlClassNameW');
        {$else}
          @LsGetViewerControlClassNameO := GetProcAddress(hDLLLS17,'LsGetViewerControlClassNameW');
      {$endif}
      @LsGetViewerControlDefaultMessage := GetProcAddress(hDLLLS17,'LsGetViewerControlDefaultMessage');
      @LsCreateViewerControlOverParent := GetProcAddress(hDLLLS17,'LsCreateViewerControlOverParent');
      {$ifdef UNICODE}
          @LlStgsysGetJobOptionStringExO := GetProcAddress(hDLLLS17,'LlStgsysGetJobOptionStringExA');
        {$else}
          @LlStgsysGetJobOptionStringEx := GetProcAddress(hDLLLS17,'LlStgsysGetJobOptionStringExA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysGetJobOptionStringEx := GetProcAddress(hDLLLS17,'LlStgsysGetJobOptionStringExW');
        {$else}
          @LlStgsysGetJobOptionStringExO := GetProcAddress(hDLLLS17,'LlStgsysGetJobOptionStringExW');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysSetJobOptionStringExO := GetProcAddress(hDLLLS17,'LlStgsysSetJobOptionStringExA');
        {$else}
          @LlStgsysSetJobOptionStringEx := GetProcAddress(hDLLLS17,'LlStgsysSetJobOptionStringExA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysSetJobOptionStringEx := GetProcAddress(hDLLLS17,'LlStgsysSetJobOptionStringExW');
        {$else}
          @LlStgsysSetJobOptionStringExO := GetProcAddress(hDLLLS17,'LlStgsysSetJobOptionStringExW');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionJobOpenO := GetProcAddress(hDLLLS17,'LsConversionJobOpenA');
        {$else}
          @LsConversionJobOpen := GetProcAddress(hDLLLS17,'LsConversionJobOpenA');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionJobOpen := GetProcAddress(hDLLLS17,'LsConversionJobOpenW');
        {$else}
          @LsConversionJobOpenO := GetProcAddress(hDLLLS17,'LsConversionJobOpenW');
      {$endif}
      @LsConversionJobClose := GetProcAddress(hDLLLS17,'LsConversionJobClose');
      @LsConversionConvertEMFToStream := GetProcAddress(hDLLLS17,'LsConversionConvertEMFToStream');
      @LsConversionConvertStgToStream := GetProcAddress(hDLLLS17,'LsConversionConvertStgToStream');
      {$ifdef UNICODE}
          @LsConversionPrintO := GetProcAddress(hDLLLS17,'LsConversionPrintA');
        {$else}
          @LsConversionPrint := GetProcAddress(hDLLLS17,'LsConversionPrintA');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionPrint := GetProcAddress(hDLLLS17,'LsConversionPrintW');
        {$else}
          @LsConversionPrintO := GetProcAddress(hDLLLS17,'LsConversionPrintW');
      {$endif}
      @LsConversionConfigurationDlg := GetProcAddress(hDLLLS17,'LsConversionConfigurationDlg');
      {$ifdef UNICODE}
          @LsConversionSetOptionStringO := GetProcAddress(hDLLLS17,'LsConversionSetOptionStringA');
        {$else}
          @LsConversionSetOptionString := GetProcAddress(hDLLLS17,'LsConversionSetOptionStringA');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionSetOptionString := GetProcAddress(hDLLLS17,'LsConversionSetOptionStringW');
        {$else}
          @LsConversionSetOptionStringO := GetProcAddress(hDLLLS17,'LsConversionSetOptionStringW');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionGetOptionStringO := GetProcAddress(hDLLLS17,'LsConversionGetOptionStringA');
        {$else}
          @LsConversionGetOptionString := GetProcAddress(hDLLLS17,'LsConversionGetOptionStringA');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionGetOptionString := GetProcAddress(hDLLLS17,'LsConversionGetOptionStringW');
        {$else}
          @LsConversionGetOptionStringO := GetProcAddress(hDLLLS17,'LsConversionGetOptionStringW');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionConvertEMFToFileO := GetProcAddress(hDLLLS17,'LsConversionConvertEMFToFileA');
        {$else}
          @LsConversionConvertEMFToFile := GetProcAddress(hDLLLS17,'LsConversionConvertEMFToFileA');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionConvertEMFToFile := GetProcAddress(hDLLLS17,'LsConversionConvertEMFToFileW');
        {$else}
          @LsConversionConvertEMFToFileO := GetProcAddress(hDLLLS17,'LsConversionConvertEMFToFileW');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionConvertStgToFileO := GetProcAddress(hDLLLS17,'LsConversionConvertStgToFileA');
        {$else}
          @LsConversionConvertStgToFile := GetProcAddress(hDLLLS17,'LsConversionConvertStgToFileA');
      {$endif}
      {$ifdef UNICODE}
          @LsConversionConvertStgToFile := GetProcAddress(hDLLLS17,'LsConversionConvertStgToFileW');
        {$else}
          @LsConversionConvertStgToFileO := GetProcAddress(hDLLLS17,'LsConversionConvertStgToFileW');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysStorageConvertO := GetProcAddress(hDLLLS17,'LlStgsysStorageConvertA');
        {$else}
          @LlStgsysStorageConvert := GetProcAddress(hDLLLS17,'LlStgsysStorageConvertA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysStorageConvert := GetProcAddress(hDLLLS17,'LlStgsysStorageConvertW');
        {$else}
          @LlStgsysStorageConvertO := GetProcAddress(hDLLLS17,'LlStgsysStorageConvertW');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysConvertO := GetProcAddress(hDLLLS17,'LlStgsysConvertA');
        {$else}
          @LlStgsysConvert := GetProcAddress(hDLLLS17,'LlStgsysConvertA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysConvert := GetProcAddress(hDLLLS17,'LlStgsysConvertW');
        {$else}
          @LlStgsysConvertO := GetProcAddress(hDLLLS17,'LlStgsysConvertW');
      {$endif}
      {$ifdef UNICODE}
          @LsMailConfigurationDialogO := GetProcAddress(hDLLLS17,'LsMailConfigurationDialogA');
        {$else}
          @LsMailConfigurationDialog := GetProcAddress(hDLLLS17,'LsMailConfigurationDialogA');
      {$endif}
      {$ifdef UNICODE}
          @LsMailConfigurationDialog := GetProcAddress(hDLLLS17,'LsMailConfigurationDialogW');
        {$else}
          @LsMailConfigurationDialogO := GetProcAddress(hDLLLS17,'LsMailConfigurationDialogW');
      {$endif}
      @LsMailJobOpen        := GetProcAddress(hDLLLS17,'LsMailJobOpen');
      @LsMailJobClose       := GetProcAddress(hDLLLS17,'LsMailJobClose');
      {$ifdef UNICODE}
          @LsMailSetOptionStringO := GetProcAddress(hDLLLS17,'LsMailSetOptionStringA');
        {$else}
          @LsMailSetOptionString := GetProcAddress(hDLLLS17,'LsMailSetOptionStringA');
      {$endif}
      {$ifdef UNICODE}
          @LsMailSetOptionString := GetProcAddress(hDLLLS17,'LsMailSetOptionStringW');
        {$else}
          @LsMailSetOptionStringO := GetProcAddress(hDLLLS17,'LsMailSetOptionStringW');
      {$endif}
      {$ifdef UNICODE}
          @LsMailGetOptionStringO := GetProcAddress(hDLLLS17,'LsMailGetOptionStringA');
        {$else}
          @LsMailGetOptionString := GetProcAddress(hDLLLS17,'LsMailGetOptionStringA');
      {$endif}
      {$ifdef UNICODE}
          @LsMailGetOptionString := GetProcAddress(hDLLLS17,'LsMailGetOptionStringW');
        {$else}
          @LsMailGetOptionStringO := GetProcAddress(hDLLLS17,'LsMailGetOptionStringW');
      {$endif}
      @LsMailSendFile       := GetProcAddress(hDLLLS17,'LsMailSendFile');
      {$ifdef UNICODE}
          @LlStgsysStorageCreateO := GetProcAddress(hDLLLS17,'LlStgsysStorageCreateA');
        {$else}
          @LlStgsysStorageCreate := GetProcAddress(hDLLLS17,'LlStgsysStorageCreateA');
      {$endif}
      {$ifdef UNICODE}
          @LlStgsysStorageCreate := GetProcAddress(hDLLLS17,'LlStgsysStorageCreateW');
        {$else}
          @LlStgsysStorageCreateO := GetProcAddress(hDLLLS17,'LlStgsysStorageCreateW');
      {$endif}
      @LlStgsysAppendEMF    := GetProcAddress(hDLLLS17,'LlStgsysAppendEMF');
      {$ifdef UNICODE}
          @LsProfileStartO := GetProcAddress(hDLLLS17,'LsProfileStartA');
        {$else}
          @LsProfileStart := GetProcAddress(hDLLLS17,'LsProfileStartA');
      {$endif}
      {$ifdef UNICODE}
          @LsProfileStart := GetProcAddress(hDLLLS17,'LsProfileStartW');
        {$else}
          @LsProfileStartO := GetProcAddress(hDLLLS17,'LsProfileStartW');
      {$endif}
      @LsProfileEnd         := GetProcAddress(hDLLLS17,'LsProfileEnd');
      @LsGetOrgTextFromGlyphsW := GetProcAddress(hDLLLS17,'LsGetOrgTextFromGlyphsW');
      {$ifdef UNICODE}
          @LsMailViewO := GetProcAddress(hDLLLS17,'LsMailViewA');
        {$else}
          @LsMailView := GetProcAddress(hDLLLS17,'LsMailViewA');
      {$endif}
      {$ifdef UNICODE}
          @LsMailView := GetProcAddress(hDLLLS17,'LsMailViewW');
        {$else}
          @LsMailViewO := GetProcAddress(hDLLLS17,'LsMailViewW');
      {$endif}
      @LsInternalCreateViewerControlOverParent13 := GetProcAddress(hDLLLS17,'LsInternalCreateViewerControlOverParent13');
      @LsInternalGetViewerControlFromParent13 := GetProcAddress(hDLLLS17,'LsInternalGetViewerControlFromParent13');
      @LsSetDlgboxMode      := GetProcAddress(hDLLLS17,'LsSetDlgboxMode');
      @LsGetDlgboxMode      := GetProcAddress(hDLLLS17,'LsGetDlgboxMode');
      @LsGetViewerControlClassNameEx := GetProcAddress(hDLLLS17,'LsGetViewerControlClassNameEx');
      @LsGetDebug           := GetProcAddress(hDLLLS17,'LsGetDebug');
      @LsConvertImageStream := GetProcAddress(hDLLLS17,'LsConvertImageStream');
      @LlStgsysStoragePrintExW := GetProcAddress(hDLLLS17,'LlStgsysStoragePrintExW');
      @LsProcessEnhMetaFileRecord := GetProcAddress(hDLLLS17,'LsProcessEnhMetaFileRecord');
      @LlStgsysGetPageData  := GetProcAddress(hDLLLS17,'LlStgsysGetPageData');
      end;
    end;
end;
...
begin
  dec(nDLLLS17Usage);
  if (nDLLLS17Usage = 0) then
    begin
    if hDLLLS17 <> 0 then
      begin
      FreeLibrary(hDLLLS17);
      hDLLLS17 := 0;
