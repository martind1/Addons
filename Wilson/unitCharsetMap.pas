unit unitCharsetMap;
(*
26.02.12 md  D2010: vieles entfernt
28.01.13 md  WideStringToUTF8 -> AnsiToUtf8Ex(Ansi, CodePage): string
             UTF8ToWideString -> Utf8ToAnsiEx(Stri, CodePage): AnsiString;
*)
interface
...
//md05.06.11:
function LCIDToRfc1766(LCID: Integer) : string;
function LCIDToLocaleName(LCID: Integer) : string;
...
var
  ...
  //MD05.06.11
  gLanguageMapLoaded : boolean = False;
  gLangMap : boolean = False;
...
var
  ...
  LanguageMap : array of TLanguageRec;

function wszToString(wsz: array of Word): string;
var
  I: integer;
begin
  for I := 0 to length(wsz) - 1 do
    result := result + WideChar(wsz[I]);
end;

procedure LoadLanguageMap;
//md05.06.11
type
  PRFC1766Info = ^tagRFC1766Info;
var
  enum : IEnumRfc1766;
  p, info : PRFC1766Info;
  i, c, ct : DWORD;
begin
  if (not gLanguageMapLoaded) or (giMultiLanguage = Nil) then
  begin
    gLanguageMapLoaded := True;
    gIMultiLanguage := CoCMultiLanguage.Create;
    gLangMap := False;

    if Assigned (gIMultiLanguage) then
    begin
      gIMultiLanguage.EnumRfc1766(enum); 
      info := CoTaskMemAlloc (10* sizeof (tagRFC1766Info));;
      try
        c := 0;
        //Next(celt: LongWord; out rgelt: tagRFC1766INFO; out pceltFetched: LongWord): HResult;
        while SUCCEEDED (enum.Next (10, info^, ct)) and (ct <> 0) do
        begin
          SetLength (LanguageMap, c + ct);
          p := info;
          for i := 0 to ct - 1 do
          begin
            LanguageMap [i + c].lcid := p^.lcid;
            LanguageMap [i + c].Rfc1766 := p^.wszRfc1766;
            LanguageMap [i + c].LocaleName  := p^.wszLocaleName;
            Inc (p);
          end;
          Inc (c, ct);
        end;
...
begin
  ...
  for i := Low (LanguageMap) to High (LanguageMap) do
    if LanguageMap[i].lcid = LCID then
    begin
      result := LanguageMap[i].Rfc1766;  //en-en
      break
    end
end;
...
begin
  ...
  for i := Low (LanguageMap) to High (LanguageMap) do
    if LanguageMap[i].lcid = LCID then
    begin
      result := LanguageMap[i].LocaleName;  //german
      break
    end
end;
...
//  begin
//    mode := 0;
//    if not SUCCEEDED (gIMultiLanguage.ConvertStringFromUnicode(
//       mode, codepage, PWideChar (ws), len, PChar (result), dlen)) then
//      dlen := 0
//  end
//  else
...
//    begin
//      mode := 0;
//      if not SUCCEEDED (gIMultiLanguage.ConvertStringToUnicode(
//         mode, codepage, PChar (st), len, PWideChar (result), dlen)) then
//        dlen := 0;
//    end
//    else
...
begin
  ...
  begin
    ...
    dlen := len * 4;
    SetLength (result, dlen);
    if not SUCCEEDED (gIMultiLanguage.ConvertString (
       mode, codepage, destCP, PChar (s), len, PChar (result), dlen)) then
      dlen := 0;
...
begin
  ...
  begin
    ...
    SetLength (result, dlen);
    if not SUCCEEDED (gIMultiLanguage.ConvertString (
       mode, srcCP, codepage, PChar (s), len, PChar (result), dlen)) then
      dlen := 0;
